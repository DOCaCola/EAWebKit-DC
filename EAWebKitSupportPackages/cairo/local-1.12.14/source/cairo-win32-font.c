			   ...
			   cairo_scaled_font_t       **font_out)
{
    ...
    //+EAWebKitChange
    //01/27/14
    f = cairo_malloc (sizeof(cairo_win32_scaled_font_t));
    //-EAWebKitChange
    if (f == NULL)
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
					  ...
					  cairo_scaled_glyph_t      *scaled_glyph)
{
    ...
    //+EAWebKitChange
    //01/27/14
    ptr = buffer = cairo_malloc (bytesGlyph);
    //-EAWebKitChange
    if (!buffer) {
	status = _cairo_error (CAIRO_STATUS_NO_MEMORY);
	goto CLEANUP_FONT;
...
cairo_win32_font_face_create_for_logfontw_hfont (LOGFONTW *logfont, HFONT font)
{
    ...
    //+EAWebKitChange
    //01/27/14
    font_face = cairo_malloc (sizeof (cairo_win32_font_face_t));
    //-EAWebKitChange
    if (!font_face) {
        _cairo_error_throw (CAIRO_STATUS_NO_MEMORY);
	goto FAIL;
