...
//2/28/2014
#if 0
// There seems to be some bug in PIXMAN_HAS_ATOMIC_OPS path due to which we run into rendering artifacts. It is kind of odd as well that 
// atomic operations are executed when we indicate that the thread safety is not required. May be the define is flipped erroneously. 
#if CAIRO_NO_MUTEX
			   ...
			   int *ix, int *iy)
{
    ...
    *ix = *iy = 0;
	...
	//+EAWebKitChange
	//01/27/14
	cleanup = cairo_malloc (sizeof (*cleanup));
	//-EAWebKitChange
					 ...
					 int *src_x, int *src_y)
{
    ...
    //+EAWebKitChange
    //01/27/14
    source = cairo_malloc (sizeof (cairo_image_source_t));
    //-EAWebKitChange
