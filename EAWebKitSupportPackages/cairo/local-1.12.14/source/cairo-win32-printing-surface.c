                                                    ...
                                                    cairo_linear_pattern_t *pattern)
{
    ...
    //+EAWebKitChange
    //01/27/14
    vert = cairo_malloc (sizeof (TRIVERTEX) * (num_rects*2*num_ranges + 4));
    //-EAWebKitChange
    //+EAWebKitChange
    //01/27/14
    rect = cairo_malloc (sizeof (GRADIENT_RECT) * (num_rects*num_ranges + 2));
    //-EAWebKitChange
				      ...
				      const cairo_clip_t    *clip)
{
	...
	//+EAWebKitChange
	//01/27/14
	dash_array = cairo_calloc (sizeof (DWORD), style->num_dashes);
	//-EAWebKitChange
...
cairo_win32_printing_surface_create (HDC hdc)
{
    ...
    //+EAWebKitChange
    //01/27/14
    surface = cairo_malloc (sizeof (cairo_win32_printing_surface_t));
    //-EAWebKitChange
    if (surface == NULL)
