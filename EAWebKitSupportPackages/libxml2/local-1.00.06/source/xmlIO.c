...
typedef struct _xmlInputCallback {
    ...
    // no read, write or close on psp2, but we do have fread and fwrite and fclose
    // we'll be using our own callbacks anyway
    int read(int context, char * buffer, int len)   { return fread( buffer, 1, len, (FILE*)context ); }
    int write(int context, const char * buffer, int len)  { return fwrite( buffer, 1, len, (FILE*)context ); }
    int close(int context)                          { return fclose( (FILE*)context ); }
