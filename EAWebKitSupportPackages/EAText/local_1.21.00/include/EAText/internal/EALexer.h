	...
	namespace Text
	{
		namespace Lexer {
			...
			struct IsSpace {
				IsSpace();
				static EALEXER_FORCE_INLINE bool match(int ch) {
					return EA::StdC::Isspace((char16_t)ch) != 0;
				}
			};
			...
			struct IsPrint {
				IsPrint();
				static EALEXER_FORCE_INLINE bool match(int ch) {
					return EA::StdC::Isprint((char16_t)ch) != 0;
				}
			};
			...
			public:
				...
				typedef eastl::basic_string<Char, EA::Allocator::EASTLICoreAllocator> string_type;
			...
			struct StaticDefinition
			{
				...
				template <typename U>
				friend bool operator<(const this_type& sd, const U* b)
				{
				...
				template <typename U>
				friend bool operator<(const U* a, const this_type& sd)
				{
