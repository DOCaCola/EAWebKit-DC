...
namespace EA
{
	...
	namespace Text
	{
		...
		{
			// Info about examples ---------------------------------------------------------------
			// The wrapping strategies shown below have two examples below each one. The '|' marks
			// represent the start and end of the layout width. The end '|' is not visible when the
			// text overflows beyond the end.
			// The example on the left shows the text in a normal case. The example on the right
			// shows the behavior in an edge case, where the first word is too long for the layout
			// width. Only the first two lines are shown in each case.
			...
			kTWNone,
			// E.g. |Extremely long lines of wrappable text!	|Extremely long lines of wrappable text!
			//		|							|				|		|
			...
			/// the 'line-break' and word-break' CSS properties. The first word will overflow if needed.
			kTWWrap,
			// E.g.	|Extremely long lines of	|				|Extremely
			//		|wrappable text!			|				|long	|
			...
			kTWSoft,
			// E.g.	|Extremely long lines of wra|				|Extreme|
			//		|ppable text!				|				|ly long|

			/// Like kTWWrap except that the first word that does not fit proceeds to go outside
			/// the bounds, instead of being wrapped to the next line.
			kTWHard,
			// E.g.	|Extremely long lines of wrappable			|Extremely
			//		|text!						|				|long lines
			...
			kTWEmergency,
			// E.g.	|Extremely long lines of	|				|Extreme|
			//		|wrappable text!			|				|ly long|
		...
		struct EATEXT_API FontStyle
		{
			...
			FontStyle();
			FontStyle(NoInit) { }
			FontStyle(const Char* pFamilyNameSpec, float fSize = 0, Style style = kStyleNormal, float weight = kWeightNormal); // pFamilyNameSpec is a semicolon-delimited set of 1 or more names.
		...
		struct EATEXT_API TextStyle : public FontStyle
		{
			...
			TextStyle();
			TextStyle(NoInit) { }
			TextStyle(const Char* pFamilyNameSpec, float fSize = 0, Style style = kStyleNormal, float weight = kWeightNormal);
		...
		public:
			...
			/// Shutdown
			/// Restores the StyleManager to a newly constructed state.
			void Shutdown();
		...
		protected:
			typedef eastl::fixed_hash_map<uint32_t, TextStyleEx*, 8, 9, true, eastl::hash<uint32_t>, eastl::equal_to<uint32_t>, false, EA::Allocator::EASTLICoreAllocator> StyleMap;
