...
namespace EA
{
	namespace Text
	{
		...
		///
		typedef eastl::vector<Rectangle> RectangleSet;
		...
		///
		struct EATEXT_API AnalysisInfo // 4+4+3=7 bytes on a 32 bit platform, 8+8+3=19 bytes on a 64 bit platform.
		{
			...
			BidiClass        mBidiClass        : 6;   /// One of enum BidiClass.
			int              mnBidiLevel       : 5;   /// Bidirectional embedding level. Range of [0, 15]. Odd numbers are left to right, even are right to left.
		...
		/// Constants for measuring the size of each member in the packed GlyphInfo struct below
		static const unsigned kGlyphInfoGlyphJustificationClassBitRange = 5;
		static const unsigned kGlyphInfoClusterPositionBitRange = 3;
		static const unsigned kGlyphInfoClusterSizeBitRange = 3;
		static const unsigned kGlyphInfoCharCountBitRange = 3;

		/// GlyphInfo
		///
		/// Glyph information which is layout-independent but is used to lay out the glyph.
		///
		struct EATEXT_API GlyphInfo  // 5 bytes
		{
			GlyphJustificationClass mGJC              : kGlyphInfoGlyphJustificationClassBitRange;  /// Justification classification
			unsigned                mbGlyphIsObject   : 1;											/// True if the glyph is a user-supplied inline object instead of a font glyph.
			unsigned                mClusterPosition  : kGlyphInfoClusterPositionBitRange;			/// Zero-based position of this glyph within its cluster. Standalone glyphs have a value of 0.
			unsigned                mClusterSize      : kGlyphInfoClusterSizeBitRange;				/// Size of the cluster this glyph is in. Standalone glyphs have a value of 1.
			...
			unsigned                mCharCount        : kGlyphInfoCharCountBitRange;				/// Number of Chars that this glyph maps to, in the case of ligatures and composed glyphs. This value is >= 1, and as of this writing, if this value > 1 then (mClusterSize == 1) and (mClusterPosition == 0).
		...
		struct EATEXT_API LineMetrics
		{
			...
			index_t  mnVisibleLineLength;   /// DEPRECATED: Use 'mnTrailingSpaceIndex' instead. Length of a given line in Unicode Chars, discounting trailing spaces (as defined by IsSpace(c, kSTAll)).  
			index_t  mnContentStartIndex;   /// The index of the first non whitespace character
			index_t  mnTrailingSpaceIndex;  /// The index of the first trailing whitepace character
			...
			float    mfVisibleSpace;        /// DEPRECATED: Use 'mfSpace - mfTrailingSpace' instead. - Space used by the mnVisibleSize characters, defined by pen position of first trailing invisible character. For paragraph layout, this will be equal to mfSpace.
			float    mfLeadingSpace;        /// The width in pixels of the leading white space		
			float    mfTrailingSpace;		/// The width in pixesl of the trailing white space
		...
		public:
			...
			FontSelection				mFontSelection;			/// Specifies the fonts currently being used by LineLayout. The primary purpose is to ensure that fonts used for the layout are not destroyed before we can use them
		...
		///
		/// Typesetter is not inherently thread-safe. If multiple threads will be 
		/// using the same typesetter then they should use a mutex to guard the 
		/// access to the Typesetter instance.
		///
		/// Example usage:
		///    Typesetter ts;
		...
		protected:
			...
			/// Read the metrics for a character, and populate GlyphInfo, GlyphMetrics and GlyphLayoutInfo, as well as returning the GlyhId
			GlyphId ReadCharacterMetrics(Char c, GlyphInfo& glyphInfo, GlyphMetrics& glyphMetrics, GlyphLayoutInfo& glyphLayoutInfo);

			/// Replaces the last glyph in a line by a hyphen
			void ReplaceSoftHyphen();
		...
		protected:
			typedef eastl::fixed_vector<const TextStyle*, 8, true, EA::Allocator::EASTLICoreAllocator> TextStyleArray;
