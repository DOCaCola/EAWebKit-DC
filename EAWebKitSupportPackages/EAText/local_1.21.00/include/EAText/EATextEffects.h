...
namespace EA
{
	namespace Text
	{
		...
		struct EATEXT_API EffectsBitmaps
		{
			...
			EA::Thread::AtomicUint32 mUserCount;                        /// Number of users who use this class. Used for knowing when the last user goes away so we can clear our memory.

			#if EATEXT_THREAD_SAFETY_ENABLED
				EA::Thread::Futex mMutex;                               /// Used for thread-safe access to the image bitmaps.
			#endif
