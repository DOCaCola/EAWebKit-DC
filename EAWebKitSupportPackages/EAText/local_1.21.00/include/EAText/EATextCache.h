...
//
// EAText offers two approaches for glyph packing. The first (legacy) is very fast but not
// very optimal when it comes to using texture space. The second (kOptionUseGlyphBinPacking)
// is the default for EAText. It is very efficient when packing glyphs but does cost 
// more in overhead due to internal tracking data (memory footprint) and it's recursive algorithm
// resulting in a higher runtime cost. Yet this algorithm is default for EAText as the cost is still
// relatively cheap compared to most other operations.
//
// First Approach (legacy):
...
// ------------------------------------------------------------------
//
// Second Approach (Rectangle Bin Packing): 
// A binary tree is maintained representing rectangles of free space in our texture.
// When a glyph is added, we perform a depth-first search for a rectangle with enough space
// to fit our glpyh and then divide the remaining area into two new rectangles into our tree. We 
// shrink the rectangle found in the search to ensure it's not re-used when performing future lookups.
//
// Texture:                          Binary Tree (width, height):
//    --------------                     R0 (24,24)
// R0 |            |                    /  \
//    |            |                  null null
//    |            |
...
//    --------------
//    
// *insert glyph 'A', width and height are 8*
//
//    --------------                    R0 (24,24)
// R0 | A |        | R2                /  \
//    -----        |           R1 (8,16)  R2 (16, 24)
...
namespace EA
{
	...
	namespace Text
	{
		...
		enum TextureFormat
		{
			...
			kTextureFormatRGB565    = 16,                   // Same as Font::kBF565.
			kTextureFormatRGB       = 24,                   // Same as Font::kBFRGB.
		...
		public:
			...
			void Init(int width, int height, int padding);
			void Reset();

			struct Node
			{
				Node() { left = right = nullptr; x = y = width = height = 0; }
			...
			/// Inserts a new rectangle of the given size into the bin. returns null if it doesn't fit.
		...
		private:
			...
			// how much gap to leave to avoid sampling issues.
			int mnGlyphPadding;
		};
		...
		struct EATEXT_API TextureInfo
		{
			...
			RectangleBinPack			mBinPack;							/// Tree of glyph bins used for glyph packing
		...
		public:
			...
			enum Option
			{
				...
				kOptionAssertOnFailure      =   7,    /// If enabled then we do assertion testing for glyph and texture allocations. Default is 1. Ignored in non-debug builds. 
				kOptionPremultipliedOutput  =   8,    /// If enabled, we output a texture using premultiplied alpha. Currently only implemented for greyscale to RGBA.
				kOptionUseGlyphBinPacking   =   9,    /// If enabled, we will maintain a tree of bins to optimally pack glyphs. Default it true.
			...
			/// TryAllocateTextureAreaBin
			///
			/// 
			bool TryAllocateTextureAreaBin(uint32_t xSize, uint32_t ySize, TextureInfo& textureInfo, uint32_t& xPosition, uint32_t& yPosition);
		...
		protected:
			...
			struct GlyphInfo
			{
				...
				GlyphInfo(Font* pFont = NULL, GlyphId glyphId = 0, float fScaleX = 0.0f, float fScaleY = 0.0f)
					: mpFont(pFont)
					, mFontCreationCounter(pFont ? pFont->GetCreationCount() : 0)
					, mGlyphId(glyphId)
					, muScaleX(EATextFloatToFixed106(fScaleX))
					, muScaleY(EATextFloatToFixed106(fScaleY))
				{
				}

				bool operator==(const GlyphInfo& gi) const
				{ 
					if((mpFont == gi.mpFont) && (mGlyphId == gi.mGlyphId) && (muScaleX == gi.muScaleX) && (muScaleY == gi.muScaleY))
					{
						if(mpFont == NULL)
						{
							return true;
						...
						// Compare the font creation counter that was saved when the glyph info was created.
						// The counter is essentially unique for each font instance that is created (it rolls over after 2^32 instances).
						// It is used to help ensure font instances are the same since it is possible for two fonts to differ even when
						// their instance pointers are the same.  For example a cache entry refer to a font instance that has been
						// deleted.  However if a new font instance is created at the same address a simple pointer comparison would
						// show they are equal when in fact they are not necessarily the same.  The counter comparison effectively
						// prevents two fonts from being considered the same in this situation.
						return mFontCreationCounter == gi.mFontCreationCounter;
					}
			...
			/// TextureInfoArray
			/// Entries are AddRefd.
			typedef eastl::fixed_vector<TextureInfo*, kTextureSlotCount, true, EA::Allocator::EASTLICoreAllocator> TextureInfoArray;
		...
		protected:
			...
			bool                       mbAssertOnFailure;                              /// See Option kOptionAssertOnFailure.
			bool                       mbPremultipliedOutput;                          /// See Option kOptionPremultipliedOutput.
			bool					   mbUseBinPack;                                   /// See Option kOptionUseGlyphBinPacking
