...
namespace EA
{
	...
	namespace Text
	{
		...
		struct EATEXT_API FontDescription
		{
			...
			FontDescription(TextStyle ts)
			  : mfSize(ts.mfSize), mStyle(ts.mStyle), mfWeight(ts.mfWeight), mfStretch(ts.mfStretch), 
				mPitch(ts.mPitch), mVariant(ts.mVariant), mSmooth(ts.mSmooth), mEffect(ts.mEffect),
				mfEffectX(ts.mfEffectX), mfEffectY(ts.mfEffectY), mEffectBaseColor(ts.mEffectBaseColor), mEffectColor(ts.mEffectColor),
				mHighLightColor(ts.mHighLightColor){ mFamily[0] = 0; }
		...
		public:
			...
			enum BitmapFormat
			{
				...
				kBF565              = 16,     /// 16 bits per pixel, no compression. Format is 0xrgb with 5 high bits r, 6 middle bits g, and 5 low bits b.
				kBFRGB              = 24,     /// 24 bits per pixel, no compression. Format is 0xrr,0xgg,0xbb.
				kBFRGBHSubpixelAA   = 28,     /// 24 bits per pixel, no compression. Format is horizontal 0xrr,0xgg,0xbb opacity values, as per horizontal sub-pixel anti-aliasing.
				kBFRGBVSubpixelAA   = 29,     /// 24 bits per pixel, no compression. Format is vertical 0xrr,0xgg,0xbb opacity values, as per vertical sub-pixel anti-aliasing.
				kBFARGB             = 32,     /// 32 bits per pixel, no compression. Format is 32 bit 0xaarrggbb in native endian.
				kBFRGBA             = 33,     /// 32 bits per pixel, no compression. Format is 32 bit 0xrrggbbaa in native endian.
				kBFABGR             = 34      /// 32 bits per pixel, no compression. Format is 32 bit 0xaabbggrr in native endian.
			};
			...
			enum Option
			{
				...
				kOptionSubPixelAA                 = 5,    /// Target LCD displays. Default is disabled.
		...
		public:
			...
			/// GetCreationCount
			///
			/// Returns the count associated with the font at its time of creation.  Normally this
			/// method should *not* be called by user code.  It is used internally by the glyph cache
			/// to compare fonts that may have the same instance pointer but actually differ because
			/// a previously cached font may have been destroyed and a new font may be allocated at
			/// the same address.
			///
			EATEXT_VIRTUAL uint32_t GetCreationCount();
		protected:
		...
		protected:
			...
			uint32_t				   mCreationCount;		// Used to compare fonts more uniquely than simply comparing addresses.
			GlyphId                    mReplacementGlyphId;

			#if EATEXT_THREAD_SAFETY_ENABLED
				EA::Thread::AtomicInt32 mRefCount;
				static EA::Thread::AtomicUint32 sCreationCounter;
			#else
				int mRefCount;
				static uint32_t sCreationCounter;
			#endif
