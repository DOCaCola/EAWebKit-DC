...
namespace EA
{
	namespace Text
	{
		...
		const Char kCharSHY     = 0x00AD;    /// 172        Soft Hyphen
		...
		/// UnicodeOptions
		///
		/// Identifies options for functions that convert text or generate text output.
		/// When you pass a NULL UnicodeOptions to a function then the function assumes
		/// default UnicodeOptions as documented below.
		///
		struct UnicodeOptions
		{
			bool mbNoNullTerminate; /// Disabled by default. When enabled, causes functions to 0-terminate strings and reserve space in the output to do so. Otherwise don't 0-terminate strings. This is useful for interfacing with some systems that require this behavior.
			char mLocale[8];        /// Empty by default. Refers to a locale context to operate within, or empty for none in particular. An example locale might be "en-us", "fr-fr", or "ja-jp". Use EA::StdC::Strlcpy to copy values into this.

			UnicodeOptions() : mbNoNullTerminate(false){ mLocale[0] = 0; }
		};
		...
		enum NormalizationType
		{
			...
			kNT_NFKD   /// Normalization Form Compatibility D.
		};
		...
		{                                           /// Supported?   Description
			...
			kCPBidiPairedBracketType,               /// +            Enumerated Unicode property Bidi_Paired_Bracket_Type. Returns enum BidiPairedBracketType values.
		...
		///
		EATEXT_API uint32_t GetCharName(Char c, Char* pName, uint32_t nNameCapacity, const UnicodeOptions* pUnicodeOptions = NULL);
