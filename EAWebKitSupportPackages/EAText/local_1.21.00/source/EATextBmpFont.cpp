...
#if EATEXT_BITMAP_USE_EAGIMEX
	#include <EAGimex/EAGimex.h>
	#include <EAGimex/EAGimexFormats.h>
#endif
...
bool BmpFont::Open(IO::IStream* pStreamArray[], uint32_t streamCount)
{
	...
	else
	{
		...
		if(pStreamArray[0])
		{
			...
			// Do some load-time validation.
			#ifdef EA_ASSERT_ENABLED
				for(BmpGlyphMetricsMap::const_iterator it = mBmpGlyphMetricsMap.begin(); it != mBmpGlyphMetricsMap.end(); ++it)
				{
					const BmpGlyphMetricsMap::value_type& value = *it;
				  //const GlyphId glyphId = value.first;
					const BmpGlyphMetrics& bmpGlyphMetrics = value.second;

					EA_ASSERT(bmpGlyphMetrics.mnTextureIndex < mTextureInfoArray.size());

					if(bmpGlyphMetrics.mnTextureIndex < mTextureInfoArray.size())
					{
						const BmpTextureInfo* pBmpTextureInfo = mTextureInfoArray[0];

						EA_ASSERT((bmpGlyphMetrics.mnPositionX < pBmpTextureInfo->mnSize) &&  
								  (bmpGlyphMetrics.mnPositionY < pBmpTextureInfo->mnSize));
					}
				}
			#endif
		}
...
bool BmpFont::ReadBmpFontInfo(IO::IStream* pStream)
{
	...
	eastl::fixed_string<char8_t,  kBufferSize + 1, true, EA::Allocator::EASTLICoreAllocator> sBuffer8(kBufferSize, 0);
	...
	while((size = EA::IO::ReadLine(pStream, &sBuffer8[0], kBufferSize)) < EA::IO::kSizeTypeDone)    
	{
		...
		if(pos < sBuffer8.length())
		{
			...
			if(EA::StdC::Stricmp(sBuffer8.c_str(), "Family") == 0)
				EA::StdC::Strlcpy(mFontDescription.mFamily, pData8, kFamilyNameCapacity); // It's not a critical problem if there wasn't enough capacity for the full string.
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Size") == 0)
				mFontDescription.mfSize = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Style") == 0)
			{
				if(EA::StdC::Stristr(pData8, "italic"))
					mFontDescription.mStyle = kStyleItalic;
				else if(EA::StdC::Stristr(pData8, "oblique"))
					mFontDescription.mStyle = kStyleOblique;
				else
					mFontDescription.mStyle = kStyleNormal;
			}
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Weight") == 0)
				mFontDescription.mfWeight = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Stretch") == 0)
				mFontDescription.mfStretch = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Smooth") == 0)
			{
				if(EA::StdC::Stristr(pData8, "Yes"))
					mFontDescription.mSmooth = kSmoothEnabled;
				else
					mFontDescription.mSmooth = kSmoothNone;
			}
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Variant") == 0)
			{
				if(EA::StdC::Stristr(pData8, "SmallCaps"))
					mFontDescription.mVariant = kVariantSmallCaps;
				else
					mFontDescription.mVariant = kVariantNormal;
			}
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "FixedPitch") == 0)
			{
				if(EA::StdC::Stristr(pData8, "Yes") == 0)
				{
			...
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "HAdvanceXMax") == 0)
				mFontMetrics.mfHAdvanceXMax = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "VAdvanceYMax") == 0)
				mFontMetrics.mfVAdvanceYMax = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Ascent") == 0)
				mFontMetrics.mfAscent = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Descent") == 0)
				mFontMetrics.mfDescent = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Leading") == 0)
				mFontMetrics.mfLeading = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "Baseline") == 0)
				mFontMetrics.mfBaseline = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "LineHeight") == 0)
				mFontMetrics.mfLineHeight = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "XHeight") == 0)
				mFontMetrics.mfXHeight = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "CapsHeight") == 0)
				mFontMetrics.mfCapsHeight = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "UnderlinePosition") == 0)
				mFontMetrics.mfUnderlinePosition = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "UnderlineThickness") == 0) // bmpFont files previously misspelled this as "UnderlingThickness".
				mFontMetrics.mfUnderlineThickness = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "LinethroughPosition") == 0)
				mFontMetrics.mfLinethroughPosition = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "LinethroughThickness") == 0)
				mFontMetrics.mfLinethroughThickness = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "GlyphMetricsMap") == 0)
			{
				...
				do{ // Read each line
					...
					do{ // Read the entries within each line.
						...
						const int       nFieldCount = EA::StdC::Sscanf(pData8, "%u %d %d %d %f %f %f %f %f", 
			...
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "KerningMap") == 0)
			{
				bool    bShouldReadAnotherLine;
				//int32_t nEntryCount = AtoI328(pData8);
				...
				do{ // Read each line
					...
					do{ // Read the entries within each line.
						...
						const int nFieldCount = EA::StdC::Sscanf(pData8, "%u %u %f", &glyphId1, &glyphId2, &kerning.mfKernX);
						(void)nFieldCount;
			...
			else if(EA::StdC::Stricmp(sBuffer8.c_str(), "CharMapSet") == 0)
			{
				bool    bShouldReadAnotherLine;
				//int32_t nEntryCount = AtoI328(pData8);
				...
				do{ // Read each line
					...
					do{ // Read the entries within each line.
						...
						const int nFieldCount = EA::StdC::Sscanf(pData8, "%u %u", &character, &glyphId);
						(void)nFieldCount;
			...
			else if(EA::StdC::Stristr(sBuffer8.c_str(), "Texture") == sBuffer8.c_str())
			{
				...
				EA::StdC::Sscanf(pData8, "%u %u %u", (unsigned*)&pBmpTextureInfo->mnTextureFileSize,
				...
				const char8_t* pTextureFileName = EA::StdC::Strchr(pData8, ' ');
				pTextureFileName = EA::StdC::Strchr(++pTextureFileName, ' ');
				pTextureFileName = EA::StdC::Strchr(++pTextureFileName, ' ');
...
bool BmpFont::ReadBmpTexture(IO::IStream* pStream, uint32_t textureIndex)
{
	...
	if(textureIndex < mTextureInfoArray.size())
	{
		...
		#if EATEXT_BITMAP_USE_EAGIMEX
			if(mbSystemMemoryCopy)
			{
				EA::Gimex::Reader reader;
				uint32_t          nFrameCount;

				pStream->AddRef(); // AddRef pStream for this function.
				reader.SetInput(pStream);

				if(reader.BeginRead(EA::Gimex::kGimexFormatID_GUESS_from_Data, nFrameCount))
				{
					EA::Gimex::Info* const pInfo = reader.GetFrameInfo(0);

					// We have already read the width and height from the .bmpFont info.
					EA_ASSERT(pBmpTextureInfo->mnSize == (unsigned)pInfo->width);
					EA_ASSERT(pBmpTextureInfo->mnSize == (unsigned)pInfo->height);

					pBmpTextureInfo->mPixelData.get_allocator().set_allocator(mpCoreAllocator);
					pBmpTextureInfo->mPixelData.resize(pInfo->width * pInfo->height);

					reader.GetFramePixels(pInfo, &pBmpTextureInfo->mPixelData[0], pBmpTextureInfo->mnSize * (pInfo->bpp / 8));
					reader.EndRead();
					reader.FreeGInfo(pInfo);
				}
				else
					bResult = false; // It's possible that bResult will be re-written as true below if we try using mpGlyphCache.

				pStream->Release(); // Release pStream for this function.
			}
		#endif
...
bool BmpFont::GetGlyphTextureInfo(GlyphId glyphId, GlyphTextureInfo& glyphTextureInfo) const
{
	...
	if(it != mBmpGlyphMetricsMap.end()) // If we have this glyph...
	{
		...
		EA_ASSERT(pBmpTextureInfo->mTexture && 
				  (bmpGlyphMetrics.mnPositionX < pBmpTextureInfo->mnSize) &&  
				  (bmpGlyphMetrics.mnPositionY < pBmpTextureInfo->mnSize));
