...
void Typesetter::PlaceGeneralGlyphCluster(eastl_size_t iCharBegin, eastl_size_t charClusterSize)
{
	...
	for(eastl_size_t g = iGlyphBegin; g < iGlyphEnd; ++g)
	{
		...
		// Possibly do a kerning adjustment.
			...
			if ((g != 0) && (glyphInfo.mbGlyphIsObject == 0)) // If we are past the first char and we are working with a character and not an object...
			{
				Kerning kerning;

				if (glyphLayoutInfo.mpFont->GetKerning(mLineLayout.mGlyphArray[g - 1], glyphId, kerning, glyphInfo.mDirection, true))
											...
											int embeddingLevel)
{
	...
	for(eastl_size_t g = 0; g < glyphClusterSize; g++)
	{
		...
		EA_ASSERT(g < (1 << kGlyphInfoClusterPositionBitRange));
		EA_ASSERT(glyphClusterSize < (1 << kGlyphInfoClusterSizeBitRange));
		EA_ASSERT(charCount < (1 << kGlyphInfoCharCountBitRange));
												...
												Char* pCharCluster, eastl_size_t& clusterSize)
{
	...
	if(EA_UNLIKELY(mLayoutSettings.mTextStyleDefault.mPasswordMode == kPMPassword)) // If the text is password text...
	{
		...
		c = kCharZWNBSP;
	else if (EA_UNLIKELY(c == kCharSHY))		// remove soft hyphens, as they can be defined in the font and will render/consume space
		c = kCharZWSP;
