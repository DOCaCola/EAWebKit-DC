...
static LineBreakCategory GetLineBreakCategoryFiltered(Char c)
{
	...
	//if(c == 0x30fC) // Temporary code for Japanese wrapping tests.
	//    lbc = kLBCAlphabetic;

	switch(lbc)
	{
		// To consider: Make EATEXT_WESTERN_IDEOGRAPHIC_LINE_BREAK_ENABLED be a runtime option controlled by some global API.
		#if EATEXT_WESTERN_IDEOGRAPHIC_LINE_BREAK_ENABLED 
		case kLBCIdeographic: // Change Ideographic to Alphabetic. Do we want to do this for all ideographic languages: Japanese (including Hiragana and Katakana), Chinese, Korean? 
		#endif 
																...
																LineBreakCategory& lbcCurrent, LineBreakCategory& lbcNext)
{
		...
		if(gpBreakDictionaryAdditionList)
		{
			for(StringList::const_iterator it = gpBreakDictionaryAdditionList->begin(), itEnd = gpBreakDictionaryAdditionList->end(); it != itEnd; ++it)
			{
				...
				const Char*   pT    = reinterpret_cast<Char*>(text);
	...
	if(!mIterator.AtEnd())
	{
		...
		if(lbcBase != kLBCLineFeed)
		{
				  ...
				  ((lbcBase != kLBCCarriageReturn) || (lbcNext == kLBCLineFeed)))
			{
				...
				// Soft hyphenation
				if (lbcNext == kLBCSoftHyphen)
				{
					lbcBase = (lineBreakType & kLineBreakTypeHyphen) ? kLBCSoftHyphen : kLBCAlphabetic;
					goto ContinueWithoutBaseUpdate;
				}
				if (lbcBase == kLBCSoftHyphen)
				{
					mLineBreakType = kLineBreakTypeHyphen;
					break;
				}
