...
void BaselineLayout::FollowBaseline(const LineLayout* pLineLayout, GlyphLayoutInfoEx* pGlyphLayoutInfoArray,
									...
									HAlignment alignment, Direction direction, float letterSpacing)
{
	const float fBaselineLength   = pBaselineIterator->GetBaselineLength();
	float       fBaselineDistance = 0.f;
	float		fRetryBaselineDistance = 0.f;
	float		fAdjustmentDistance = 0.f;
	bool		bRetry = false;
	...
	do
	{
		fRetryBaselineDistance = fBaselineDistance;
		...
		for (eastl_size_t i = 0, iEnd = pLineLayout->mGlyphLayoutInfoArray.size(); i < iEnd; i++)
		{
			...
			do {
				...
				const float adjustment = (fGlyphHeight / 64);  // This constant here is somewhat arbitrary. It should be a value between ~4 and 128.
				fAdjustmentDistance += adjustment;
				fBaselineDistance += adjustment;

				iterations++;
			} while (fBaselineDistance < fMaxDistance && iterations < kMaxIterations);
			...
			fBaselineDistance += gli.mfAdvance + letterSpacing;
		}

		if (alignment == kHACenter)
		{
			// In centered text we might have needed to adjust the curved text forcing it to not be centered anymore
			if (fAdjustmentDistance > 0.f && !bRetry)
			{
				// Move the baseline back by the amount of extra space we pushed to the right to try to
				// center the text better the next time we place our glyphs
				fBaselineDistance = fRetryBaselineDistance - fAdjustmentDistance;
				fAdjustmentDistance = 0.f;

				// We might still be off center but we should be better than before. We stop retrying because if we 
				// keep adjusting in this manner we will push our text unpleasantly to the left and become offcenter.
				// If this isn't good enough for your usage please contact Frostbite.GFW.Foundation Support
				bRetry = true;
			}
			else
			{
