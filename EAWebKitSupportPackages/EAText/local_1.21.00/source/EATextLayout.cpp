...
namespace Text
{
// If this assets we have run out of bits to store into our GlyphInfo safely
EAT_COMPILETIME_ASSERT(kGJCCount <= (1<<kGlyphInfoGlyphJustificationClassBitRange));
...
void Layout::SetLayoutFonts(const TextStyle* pTextStyle, LayoutSettings* pLayoutSettings, FontServer* pFontServer)
{
	...
	const uint32_t kMaxFonts = 32;
	pFontServer->GetFont(pTextStyle, pLayoutSettings->mFontSelection, kMaxFonts, kCharInvalid, kScriptLatin, true);
}
...
{
	static AnalysisArray sLayoutTextLineAnalysysArray(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "Layout", EA::Text::GetAllocator())); // this is protected by the mutex inside
...
{
	#if EATEXT_THREAD_SAFETY_ENABLED
		EA::Thread::AutoFutex autoMutex(gLayoutScratchpad.mMutex);
	#endif
...
{
	static AnalysisArray sLayoutTextParagraphAnalysisArray(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "Layout", EA::Text::GetAllocator()));  // this is protected by the mutex inside
...
{
	static AnalysisArray sCalculateLineBreakAnalysysArray(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "Layout", EA::Text::GetAllocator())); // this is protected by the mutex inside
