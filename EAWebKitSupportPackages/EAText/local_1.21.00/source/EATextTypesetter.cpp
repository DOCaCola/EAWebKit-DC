...
namespace
{
	/// If we're rendering a single line of text without wrapping, analyze this many characters
	const eastl_size_t kNonWrapAnalysisSize = 16384;
	/// When dealing with wrapping, always analyze at least this many characters
	const eastl_size_t kMinAnalysisSize = 200;
	/// There are different types of Unicode bullets. Not all fonts support  bullet type.
	/// When the typesetter is in password mode we want to check for both.
	const Char g_passwordChars[] = { 0x25CF, 0x2022, 0x2219, 0x2B24, 0x2218, 0x25E6, '*' };

	/// SpanList
	///
	/// Used to store the visual extents of a span of glyphs.
	///
	typedef eastl::list<Span> SpanList;
	...
	void AddRectangleToRectangleSet(const Rectangle& rect, RectangleSet& rectangleSet)
	{
		...
		rectangleSet.push_back(unionRect);
	}
...
LineLayout::LineLayout(Allocator::ICoreAllocator* pCoreAllocator)
  : mpCoreAllocator(pCoreAllocator ? pCoreAllocator : EA::Text::GetAllocator()),
	...
	mFontSelection(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "Typesetter/FontSelection", mpCoreAllocator)),
...
void LineLayout::SetAllocator(Allocator::ICoreAllocator* pCoreAllocator)
{
	...
	mFontSelection		 .get_allocator().set_overflow_allocator(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "Typesetter/FontSelection", pCoreAllocator));
...
void LineLayout::Assign(const LineLayout& lineLayout, int componentFlags)
{
	...
	// rebuild mFontSelection set based on fonts currently in use
	FontSelection newFS(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "Typesetter/FontSelection", mpCoreAllocator));
	for (GlyphLayoutInfoArray::iterator it = mGlyphLayoutInfoArray.begin(), end = mGlyphLayoutInfoArray.end(); it != end; ++it)
	{
		newFS.insert(it->mpFont);
	}
	for (AnalysisInfoArray::iterator it = mAnalysisInfoArray.begin(), end = mAnalysisInfoArray.end(); it != end; ++it)
	{
		newFS.insert(it->mpFont);
	}
	mFontSelection = newFS;
...
void LineLayout::Append(const LineLayout& lineLayout, int componentFlags)
{
	...
	mLineMetrics.mnVisibleLineLength     = 0; // This number is meaningless across multiple lines. DEPRECATED: Remove this when struct field LineMetrics.mnVisibleLineLength is removed.
	mLineMetrics.mnContentStartIndex     = 0; // This number is meaningless across multiple lines.
	mLineMetrics.mnTrailingSpaceIndex    = 0; // This number is meaningless across multiple lines.
	mLineMetrics.mfSpace                 = eastl::max_alt(mLineMetrics.mfSpace,         lineLayout.mLineMetrics.mfSpace);
	mLineMetrics.mfVisibleSpace          = eastl::max_alt(mLineMetrics.mfVisibleSpace,  lineLayout.mLineMetrics.mfVisibleSpace); // DEPRECATED: Remove this when struct field LineMetrics.mfVisibleSpace is removed.
	mLineMetrics.mfLeadingSpace          = eastl::max_alt(mLineMetrics.mfLeadingSpace,  lineLayout.mLineMetrics.mfLeadingSpace); // Maximum leading space from the appended lines. Useless?
	mLineMetrics.mfTrailingSpace         = eastl::max_alt(mLineMetrics.mfTrailingSpace, lineLayout.mLineMetrics.mfTrailingSpace); // Maximum trailing space from the appended lines. Useless?
	...
	mFontSelection.insert(lineLayout.mFontSelection.begin(), lineLayout.mFontSelection.end());
}
...
//     mLineMetrics.mnVisibleLineLength - DEPRECATED: Remove this when struct field LineMetrics.mnVisibleLineLength is removed.
//     mLineMetrics.mnContentStartIndex
//     mLineMetrics.mnTrailingSpaceIndex
//     mLineMetrics.mfVisibleSpace - DEPRECATED: Remove this when struct field LineMetrics.mfVisibleSpace is removed.
//     mLineMetrics.mfLeadingSpace
//     mLineMetrics.mfTrailingSpace
//
void LineLayout::CalculateVisibleSpace()
{
	eastl_size_t iEnd, iStart; 

	// Calculate which character is the first trailing non-whitespace character (i.e. mnTrailingSpaceIndex).
	...
	for(iEnd = mLineMetrics.mnLineLength; iEnd > 0; --iEnd)
	{
		...
		// 0x2029 is Paragraph Separator which we need to skip over to get to the trailing spaces
		if((c != 0x0020) && (c != 0x00A0) && (c != 0x3000) && (c != 0x2029)) // 0x00A0 -> kCharNBSP; 0x3000 -> kCharIDSP 
			break;
	}

	// Calculate which character is the first visible character (i.e. mnContentStartIndex).
	for (iStart = 0; iStart < mLineMetrics.mnLineLength; iStart++)
	{
		const Char c = mCharArray[iStart];

		if ((c != 0x0020) && (c != 0x00A0) && (c != 0x3000)) // 0x00A0 -> kCharNBSP; 0x3000 -> kCharIDSP
			break;
	}

	mLineMetrics.mnVisibleLineLength = iEnd; // DEPRECATED: Remove this when struct field LineMetrics.mnVisibleLineLength is removed.
	mLineMetrics.mnContentStartIndex = iStart;
	mLineMetrics.mnTrailingSpaceIndex = iEnd;

	// Calculate what the extent of the preceding glyph is (i.e. mfVisibleSpace).
	const eastl_size_t iGlyphStart  = GetGlyphIndexFromCharIndex(iStart);
	const eastl_size_t iGlyphEnd    = GetGlyphIndexFromCharIndex(iEnd);

	if(iGlyphEnd > 0 && iEnd != 0) // for text that is either empty or all spaces, iEnd will equal 0 and go to special cases section below
	{
		// This isn't correct, though it will work most of the time. We need to go back and find the base char of clusters.
		GlyphLayoutInfo& gliFirst   = mGlyphLayoutInfoArray[iGlyphStart];
		GlyphLayoutInfo& gliLast    = mGlyphLayoutInfoArray[iGlyphEnd - 1];
		mLineMetrics.mfLeadingSpace = gliFirst.mfPenX;
		float trailingSpaceStart = gliLast.mfAdvance > 0 ? gliLast.mfPenX + gliLast.mfAdvance : gliLast.mfPenX;
		mLineMetrics.mfTrailingSpace = mLineMetrics.mfSpace - trailingSpaceStart;
		mLineMetrics.mfVisibleSpace = trailingSpaceStart; // DEPRECATED: Remove this when struct field LineMetrics.mfVisibleSpace is removed.
	}
	else // special cases
	{
		if (iStart != 0) // text is all spaces
		{
			mLineMetrics.mnContentStartIndex = 0;
			mLineMetrics.mfTrailingSpace = mLineMetrics.mfSpace;
		}
		else // text is empty (or unexpected scenario)
		{
			mLineMetrics.mfTrailingSpace = 0;
		}

		mLineMetrics.mfVisibleSpace = 0; // DEPRECATED: Remove this when struct field LineMetrics.mfVisibleSpace is removed.
		mLineMetrics.mfLeadingSpace = 0;
	}
}
...
{
	// if we are including only the visible glyphs
	if(bVisible)
	{
		// calculate the visible width if it hasn't been already - includes leading whitespace
		if(mLineMetrics.mfSpace - mLineMetrics.mfTrailingSpace == 0)
			CalculateVisibleSpace();
		w = mLineMetrics.mfSpace - mLineMetrics.mfTrailingSpace;
	}
	else
	{
		// otherwise use the width value which includes leading and trailing whitespace
...
bool LineLayout::Validate(bool bValidateSync)
{
	...
	if(bValidateSync)
	{
		...
		{
			// TODO: This asserts in Char32 test. The GetGlyphIndexFromCharIndex inside of FinalizeLine resizes all the glpyh and char arrays to be
			// the size of what it thinks should be the end of the line. Either the index it is using is wrong, or the data inside of the glyph index is wrong.
			// This assert has existed since at least 1.14.02. At this point it is known shippable. We should look into fixing this at some point. However, no one is using char32 atm.
			// This is not high priority. Frostbite is using utf8 and utf16. The localization team wants us to use utf8 only.
...
Typesetter::Typesetter(Allocator::ICoreAllocator* pCoreAllocator)
 :  mpCoreAllocator(pCoreAllocator ? pCoreAllocator : EA::Text::GetAllocator()),
	...
	mSchedule(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "Typesetter/Typesetter", mpCoreAllocator)),
	mLineBegin(),
	mLineEnd(),
	mAnalysisEnd(),
	mScheduleEnd(),
...
void Typesetter::SetAllocator(Allocator::ICoreAllocator* pCoreAllocator)
{
	...
	// Set the overflow allocator of the text style array for the rare case that it needs to resize
	mTextStyleArray.get_allocator().get_overflow_allocator().set_allocator(pCoreAllocator);
...
void Typesetter::SetLayoutSettings(LayoutSettings& layoutSettings)
{
	...
	if(!mLayoutSettings.mLocale[0])
		EA::StdC::Strcpy(mLayoutSettings.mLocale, EATEXT_DEFAULT_LOCALE);
...
void Typesetter::FinalizeLine()
{
	...
	//     - mnVisibleLineLength    : Need to calculate  DEPRECATED: Remove this when struct field LineMetrics.mnVisibleLineLength is removed.
	//     - mnContentStartIndex    : Need to calculate
	//     - mnTrailingSpaceIndex   : Need to calculate
	//     - mfSpace                : Already set
	//     - mfVisibleSpace         : Need to calculate  DEPRECATED: Remove this when struct field LineMetrics.mfVisibleSpace is removed.
	//     - mfLeadingSpace         : Need to calculate
	//     - mfTrailingSpace        : Need to calculate
	...
	{
		EA_ASSERT(0 < lineLength);

		// Calculate the space used by the glyphs before doing the char array resize below. 
		// Otherwise the GetGlyphIndexFromCharIndex would yield incorrect 
		// values, because it's dependent on those sizes.
		const eastl_size_t glyphLength = mLineLayout.GetGlyphIndexFromCharIndex(lineLength - 1) + 1;
		// Note: The glyph index of the last character will be the index of the last glyph

		// Chop off glyph info that refers to glyphs past the end of this line.
		mLineLayout.mGlyphArray.resize(glyphLength);
		mLineLayout.mGlyphInfoArray.resize(glyphLength);
		mLineLayout.mGlyphLayoutInfoArray.resize(glyphLength);
		mLineLayout.mCharIndexArray.resize(glyphLength);
...
void Typesetter::UpdateAnalysis()
{
	...
	eastl_size_t analysisSize = kNonWrapAnalysisSize;
	if (mpCurrentTextStyle->mTextWrap != kTWNone)
	{
		// Let's say the worst case is every glyph ends up only 1 pixel wide with no spaces between glyphs.
		// In that case we'd better check at least as many chars as there are pixels in our layout area
		// But checking more (like say 16k) chars when we're rendering a 200-pixel-wide line is totally nonsense
		analysisSize = static_cast<eastl_size_t>(mfLayoutSpace);
		// But always check at least this many chars
		analysisSize = analysisSize < kMinAnalysisSize ? kMinAnalysisSize : analysisSize;
	}

	// We append additional characters to mLineLayout.mCharArray and mLineLayout.mAnalysisInfoArray.
	while(((analysisInfoArraySize = mLineLayout.mAnalysisInfoArray.size()) < analysisSize) && // While analysis size < max analysis size and
		  (mAnalysisEnd.mnScheduleIndex < mScheduleEnd.mnScheduleIndex))                          // while there are any items left to analyze...
	{
		...
		switch (item.mType)
		{
			...
			case kItemTypeText:
			{
				...
				EA_ASSERT(analysisInfoArraySize < analysisSize);

				const index_t nMaxCount = (analysisSize - analysisInfoArraySize);
				...
				if(nCount > nMaxCount) // If this string is long and causes mLineLayout.mAnalysisInfoArray to go over analysisSize...
				{
				...
				// OK, currently we apply all the text in this segment, even if it goes over analysisSize. 
				while(pCurrent < pEnd)
				{
...
void Typesetter::UpdateFontSelection(eastl_size_t iCharBegin, eastl_size_t iCharEnd)
{
	...
	if(mLayoutSettings.mbDynamicFontSelection)
	{
		...
		if(mLayoutSettings.mTextStyleDefault.mPasswordMode == kPMPassword) // If the text is password text...
		{
			...
			mLineLayout.mFontSelection.insert(pFont);

			for (int i = 0, size = sizeof(g_passwordChars); i < size; i++)
			{
				if (pFont->GetGlyphIds(&g_passwordChars[i], 1, &glyphId, false))
				{
					mPasswordChar = g_passwordChars[i];
					break;
				}
			}
			EA_ASSERT_MSG(mPasswordChar != 0, "The font couldn't find a valid character for mPasswordChar. You might need to modify EAText to add an additional fallback.");
		...
		else
		{
			...
			for(eastl_size_t i = iCharBegin; i < iCharEnd; i++)
			{
				...
				const TextStyle* pNextTextStyle = mLineLayout.mAnalysisInfoArray[i].mpTextStyle;

				if(pNextTextStyle == pCurrentTextStyle)  // If using the same style as the last char...
				{
					if(pCurrentFont) // In practice this should always be so due to the logic here, but we check anyway.
					{
						if(!pCurrentFont->IsCharSupported(c, script)) // If the font doesn't support this char, do a new lookup.
						{
							...
							mLineLayout.mFontSelection.insert(pCurrentFont);
				...
				{
					pCurrentTextStyle = pNextTextStyle;
					pCurrentFont = mpFontServer->GetFont(pCurrentTextStyle, NULL, 0, c, script, true);
					mLineLayout.mFontSelection.insert(pCurrentFont);
					pCurrentFont->Release(); // Matches the AddRef that GetFont did. 
				}
...
void Typesetter::SetGlyphLayoutInfo(GlyphLayoutInfo& glyphLayoutInfo, float fPenX, const GlyphMetrics& glyphMetrics)
{
	...
	namespace
	{
		...
		{ 
			{ EATEXT_CHAR("ffl"), 0xFB04 }, // Note that we place ffl before ff, so as to catch it first.
			{ EATEXT_CHAR("ffi"), 0xFB03 }, 
			{ EATEXT_CHAR("fl"),  0xFB02 }, 
			{ EATEXT_CHAR("fi"),  0xFB01 }, 
			{ EATEXT_CHAR("ff"),  0xFB00 }, 
			{ EATEXT_CHAR("oe"),  0x0153 }, 
			{ EATEXT_CHAR("OE"),  0x0152 }, 
			{ EATEXT_CHAR("ij"),  0x0133 }, 
			{ EATEXT_CHAR("IJ"),  0x0132 },
		};
...
void Typesetter::UpdateLineState()
{
	...
	if(nCurrentBreakIndex < nTextLength)
	{
		...
		{
			const bool bCurrentBreakUsesSoftHyphen(lineBreakType == kLineBreakTypeHyphen);		// store if we were looking at a soft hyphen at the current break location
			...
			float fSoftHyphenSpace = 0;
			if ((lineBreakType == kLineBreakTypeHyphen)		// if this is a soft hyphen, we need to consider it for the current block, as it might push it over the boundary, and thus not be applicable
				&& (textWrap != kTWSoft))					// note: soft wrap doesn't use hyphens, but instead just runs through to the next line when wrapping
			{
				if (fHyphenAdvance == 0.f)					// load and cache the width of the actual hyphen character
				{
					Char            c = EATEXT_CHAR("-")[0];			// kCharHyphen does not seem to work
					GlyphInfo		glyphInfo;
					GlyphMetrics    glyphMetrics;
					GlyphLayoutInfo glyphLayoutInfo;
					ReadCharacterMetrics(c, glyphInfo, glyphMetrics, glyphLayoutInfo);
					fHyphenAdvance = glyphMetrics.mfHAdvanceX;
				}
				fSoftHyphenSpace = fHyphenAdvance;
			}

			if((fNextBreakWidth + fSoftHyphenSpace > fLayoutWidth) && (textWrap != kTWNone)) // If this last break goes over the alotted space...
			{
				...
				else if((textWrap == kTWSoft) || ((textWrap == kTWEmergency) && (nCurrentBreakIndex == 0))) // If we are to break on grapheme cluster boundaries instead of word boundaries...
				{
					...
					nNextBreakIndex = nCurrentBreakIndex;							// Go back to the previous break
					fNextBreakWidth = GetTextWidth(nNextBreakIndex);
					index_t nSavedBreakIndex(nNextBreakIndex);
					float   fSavedBreakWidth(fNextBreakWidth);

					while (fNextBreakWidth <= fLayoutWidth)
					{
					...
					if (nSavedBreakIndex == 0)										// The very first character doesn't fit!
					{
						mLineLayout.mLineMetrics.mnLineLength = nNextBreakIndex;	// Get the last known content that fit
						mLineLayout.mLineMetrics.mfSpace      = fNextBreakWidth;
						EA_ASSERT(nNextBreakIndex != 0);							// If we get this assert, then we're headed for an infinite loop!
					}
					else															// At least one character fit
					{
						mLineLayout.mLineMetrics.mnLineLength = nSavedBreakIndex;	// Get the last known content that fit
				...
				else // (textWrap == kTWWrap) || ((textWrap == kTWEmergency) && (nCurrentBreakIndex > 0))
				{
					...
					{
						// Technically, this means that nothing fit, but if we return an empty line, we'll get stuck in a loop, so we're going to allow overflow here
						mLineLayout.mLineMetrics.mnLineLength = nNextBreakIndex;
						mLineLayout.mLineMetrics.mfSpace	  = fNextBreakWidth;
					}
					if (bCurrentBreakUsesSoftHyphen)
						ReplaceSoftHyphen();
				}
...
GlyphId Typesetter::ReadCharacterMetrics(Char c, GlyphInfo& glyphInfo, GlyphMetrics& glyphMetrics, GlyphLayoutInfo& glyphLayoutInfo)
{
	...
	GetGlyphsForChar(&c, 1, pAnalysisInfo, glyphIdArray, glyphIdCount, EATEXT_CHAR("_"), 1);
	...
	glyphInfo.mDirection = kDirectionLTR;
...
void Typesetter::ReplaceSoftHyphen()
{
	Char            c = EATEXT_CHAR("-")[0];			// kCharHyphen does not seem to work
	GlyphInfo		glyphInfo;
	GlyphMetrics    glyphMetrics;
	GlyphLayoutInfo glyphLayoutInfo;
	GlyphId glyphId = ReadCharacterMetrics(c, glyphInfo, glyphMetrics, glyphLayoutInfo);

	// this is mostly the same as SubstituteEllipsis, but simpler as we already know what to replace
	EA_ASSERT(mLineLayout.mLineMetrics.mnLineLength > 0);
	eastl_size_t hyphenIndex = mLineLayout.mLineMetrics.mnLineLength - 1;
	const float fPenX= mLineLayout.mGlyphLayoutInfoArray[hyphenIndex].mfPenX;
	const float fPenY = mLineLayout.mGlyphLayoutInfoArray[hyphenIndex].mfPenY;

	glyphInfo.mDirection = mLineLayout.mGlyphInfoArray[hyphenIndex].mDirection;
	glyphLayoutInfo.mfPenX += fPenX;
	glyphLayoutInfo.mfX1 += fPenX;
	glyphLayoutInfo.mfX2 += fPenX;
	glyphLayoutInfo.mfPenY += fPenY;
	glyphLayoutInfo.mfY1 += fPenY;
	glyphLayoutInfo.mfY2 += fPenY;
	mLineLayout.mGlyphArray[hyphenIndex] = glyphId;
	mLineLayout.mGlyphInfoArray[hyphenIndex] = glyphInfo;
	mLineLayout.mGlyphLayoutInfoArray[hyphenIndex] = glyphLayoutInfo;
	mLineLayout.mCharArray[hyphenIndex] = c;

	mLineLayout.mLineMetrics.mfSpace += glyphMetrics.mfHAdvanceX;
}
...
void Typesetter::SubstituteEllipsis()
{
	...
	if(mLineLayout.mLineMetrics.mfSpace > mfLayoutSpace)
	{
		...
		mLineLayout.mGlyphArray[ellipsisGlyphIndex]           = glyphId;
	...
	switch (mLayoutSettings.mTextStyleDefault.mHAlignment)
	{
		...
		case kHARight:
		{
			if (mLayoutSettings.mDirection == kDirectionLTR)
				fXOffset += mfLayoutSpace - mLineLayout.mLineMetrics.mfSpace + mLineLayout.mLineMetrics.mfTrailingSpace;
			// Else we do nothing because in RTL text, left and right are reversed.
			break;
		}
		case kHACenter:
		{
			float centerSpace = mfLayoutSpace - mLineLayout.mLineMetrics.mfSpace - mLineLayout.mLineMetrics.mfLeadingSpace + mLineLayout.mLineMetrics.mfTrailingSpace;
			fXOffset += floorf(centerSpace / 2);
			break;
		}
		case kHAJustify:
		{
...
void Typesetter::Justify()
{
	...
	const float fExtraSpace = mfLayoutSpace - mLineLayout.mLineMetrics.mfSpace + mLineLayout.mLineMetrics.mfTrailingSpace;

	// spaceIndexArray is a list of all characters that are candidates for inter-word spacing extension.
	eastl::fixed_vector<eastl_size_t, 64, true, EA::Allocator::EASTLICoreAllocator> spaceIndexArray; // To consider: Make this a member variable.
	...
	for(eastl_size_t i = 0, iEnd = mLineLayout.mLineMetrics.mnTrailingSpaceIndex; i < iEnd; i++)
	{
		const Char c = mLineLayout.mCharArray[i];
...
void Typesetter::ReorderBidiRunInfoArray(RunInfoArray& runInfoArray)
{
		...
		{
			// Indexes into mBidiRunInfoArray. Used later for eastl::reverse: "reverse any contiguous 
			// sequence of characters that are at that level or higher"
			eastl::vector<eastl_size_t> indexArray;
			...
			for(int e = (highestLevel - 1); e >= lowestOddLevel; e--)
			{
				...
				for(eastl::vector<eastl_size_t>::iterator it = indexArray.begin(), itEnd = indexArray.end(), itContigEnd; it != itEnd; it = itContigEnd)
				{
									...
									const TextStyle* pTextStyle, LineLayout& lineLayout, int componentFlags)
{
	...
	while(lineState != Typesetter::kLineStateComplete)
	{
		...
		const float fLineHeight = mLayoutSettings.mTextStyleDefault.mfLineSpace > 0 ? mLayoutSettings.mTextStyleDefault.mfLineSpace : (float)ceilf(mLineLayout.mLineMetrics.mfBaseline - mLineLayout.mLineMetrics.mfDescent);
		if(bFitPixelGrid)
		{
	...
	{
		mLineLayout.Clear();
		mLineLayout.Append(lineLayoutTemp);
	}
											 ...
											 const TextStyle* pTextStyle, float fLayoutWidth)
{
	...
	while(GetLineState() != Typesetter::kLineStateComplete)
	{
		...
		result.mX = eastl::max_alt(result.mX, layout.mLineMetrics.mfSpace - layout.mLineMetrics.mfTrailingSpace);
		NextLine(false);
	}
