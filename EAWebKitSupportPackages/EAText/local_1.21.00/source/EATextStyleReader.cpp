...
TextStyleReader::TextStyleReader(StyleManager* pManager)
	...
	, mTokenLineStart(0)
	, mTokenPos(0)
	, mTokenValue(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "TextStyleReader", EA::Text::GetAllocator()))
	, mpCallback(NULL)
...
{
	if ((mToken == TokenIdent) && (EA::StdC::Strcmp(pSymbol, MatchText().c_str()) == 0))
	{
		Advance();
		return true;
	}
...
bool TextStyleReader::MatchFloat(float& result)
{
	...
	{
		result = float(EA::StdC::Strtod(MatchText().c_str(), NULL));
		Advance();
		return true;
	}
	return false;
...
bool TextStyleReader::MatchInteger(uint32_t& result)
{
	...
	{
		result = EA::StdC::StrtoU32(MatchText().c_str(), NULL, 10);
		Advance();
		return true;
	}
	...
	{
		result = EA::StdC::StrtoU32(MatchText().c_str(), NULL, 16);
		Advance();
		return true;
	}
	return false;
...
bool TextStyleReader::MatchColor(Color& color)
{
	...
	eastl::basic_string<char, EA::Allocator::EASTLICoreAllocator>& matchText = MatchText();
	...
	if (mToken == TokenPound)
	{
		...
		uint32_t n = EA::StdC::StrtoU32(matchText.c_str(), NULL, 16);
...
bool TextStyleReader::ParseFontFamilies(TextStyle& style)
{
	...
	while (MatchToken(TokenIdent) || MatchToken(TokenString))
	{
		...
		{
			EA::StdC::Strlcpy(&style.mFamilyNameArray[numFonts][0], mTokenValue.c_str(), kFamilyNameCapacity);
			numFonts++;
		}
...
bool TextStyleReader::ParseFontEffect(TextStyle& style) 
{
	...
	else if (MatchSymbol("effectID")) // Look for a hex GUID.
	{
		...
		{
			style.mEffect = EA::StdC::FNV1(mTokenValue.c_str(), mTokenValue.length());
			return true;
		}
...
bool TextStyleReader::ReadStyle(char8_t* pStyleName, int32_t nStyleNameCapacity, uint32_t& nStyleID, TextStyle& style) 
{
	...
	{
		eastl::fixed_string<Char, 256, true, EA::Allocator::EASTLICoreAllocator>        buffer;       // buffer look like this: "abc\0123\defghi\0"
		eastl::fixed_vector<eastl_size_t, 16, true, EA::Allocator::EASTLICoreAllocator> argPosArray;
		...
		if (mpCallback)
		{
			eastl::fixed_vector<const Char*, 16, true, EA::Allocator::EASTLICoreAllocator> argArray;
	...
	if (MatchToken(TokenIdent) || MatchToken(TokenString))
	{
		EA::StdC::Strlcpy(pStyleName, mTokenValue.c_str(), nStyleNameCapacity);
		EA::StdC::Strlcpy(style.mStyleName, mTokenValue.c_str(), EAArrayCount(style.mStyleName));
	...
	if (MatchToken(TokenOpenBrace))
	{
		while (!AtEoF())
		{
			...
			{
				EA::StdC::Strlcpy(style.mStyleName, pStyleName, EAArrayCount(style.mStyleName));
				return true;
			}
		...
		// If there's no trailing white space, the above while loop will exit before accepting the last closing brace, so we'll try to match it again here.
		if (MatchToken(TokenCloseBrace))
		{
			EA::StdC::Strlcpy(style.mStyleName, pStyleName, EAArrayCount(style.mStyleName));
			return true;
