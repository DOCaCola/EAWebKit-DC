...
struct FontFileLineReader
{
	...
	eastl::fixed_string<char8_t,  kBufferSize + 1, true, EA::Allocator::EASTLICoreAllocator> mBuffer8;
...
PolygonGlyph::PolygonGlyph()
  : mnBackFaceVertexStart(0),
	...
	mVertexArray(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "PolygonGlyph")),
	mIndexArray(EA::Allocator::EASTLICoreAllocator(EATEXT_ALLOC_PREFIX "PolygonGlyph"))
{
	// Empty
}
...
bool PolygonFont::Open(IO::IStream* pStream)
{
	...
	while((size = reader.ReadLine(pStream)) < EA::IO::kSizeTypeDone)    
	{
		...
		if(pos < reader.mBuffer8.length())
		{
			...
			if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Family") == 0)
				EA::StdC::Strlcpy(mFontDescription.mFamily, pData8, kFamilyNameCapacity);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Size") == 0)
				mFontDescription.mfSize = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Style") == 0)
			{
			...
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Weight") == 0)
				mFontDescription.mfWeight = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Stretch") == 0)
				mFontDescription.mfStretch = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Smooth") == 0)
			{
			...
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Variant") == 0)
			{
			...
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "FixedPitch") == 0)
			{
				if(EA::StdC::Stristr(pData8, "Enabled") == 0)
				{
			...
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "HAdvanceXMax") == 0)
				mFontMetrics.mfHAdvanceXMax = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "VAdvanceYMax") == 0)
				mFontMetrics.mfVAdvanceYMax = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Ascent") == 0)
				mFontMetrics.mfAscent = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Descent") == 0)
				mFontMetrics.mfDescent = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Leading") == 0)
				mFontMetrics.mfLeading = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "Baseline") == 0)
				mFontMetrics.mfBaseline = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "LineHeight") == 0)
				mFontMetrics.mfLineHeight = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "XHeight") == 0)
				mFontMetrics.mfXHeight = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "CapsHeight") == 0)
				mFontMetrics.mfCapsHeight = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "UnderlinePosition") == 0)
				mFontMetrics.mfUnderlinePosition = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "UnderlineThickness") == 0) // bmpFont files previously misspelled this as "UnderlingThickness".
				mFontMetrics.mfUnderlineThickness = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "StrikethroughPosition") == 0)
				mFontMetrics.mfLinethroughPosition = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "StrikethroughThickness") == 0)
				mFontMetrics.mfLinethroughThickness = (float)EA::StdC::Atof(pData8);
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "GlyphMetricsMap") == 0)
			{
				...
				do{ // Read each line
					...
					do{ // Read the entries within each line.
						unsigned        glyphId;
						GlyphMetrics    gm;
						const int       nFieldCount = EA::StdC::Sscanf(pData8, "%u %f %f %f %f %f", 
			...
			else if(EA::StdC::Stricmp(reader.mBuffer8.c_str(), "CharMapSet") == 0)
			{
				bool bShouldReadAnotherLine;
			...
			else if(EA::StdC::Stristr(reader.mBuffer8.c_str(), "Glyph") == reader.mBuffer8.c_str())
			{
				if(!mbUserGlyphInfo)
				{
					Allocator::ICoreAllocator* const pAllocator = mPolygonGlyphMap.get_allocator().get_allocator();

					const uint32_t glyphId = EA::StdC::AtoU32(reader.mBuffer8.c_str() + 5); // reader.mBuffer8 looks something like "Glyph37: # # # #, array array"
					...
					const int nFieldCount = EA::StdC::Sscanf(pData8, "%u %u %u %u", &vertexArraySize, &indexArraySize, &backFaceVertexStart, &backFaceIndexStart);
					...
					if(glyphPtr.mpPolygonGlyph)
					{
						...
						if(vertexArraySize) // Some glyphs have no geometry (e.g. the space char)
						{
							...
							pData8 = EA::StdC::Strchr(pData8, ',') + 1;
