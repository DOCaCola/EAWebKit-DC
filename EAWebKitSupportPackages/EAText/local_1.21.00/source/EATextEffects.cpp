...
EATEXT_API uint32_t CreateInstructionList(const char* pInstructionString, EA::Text::EIWord* pInstructionList, uint32_t /*instructionListCapacity*/, int32_t* pErrorLine)
{
	...
	do{
		pLine    = pLineNext;
		pLineEnd = EA::StdC::GetTextLine(pLine, pInstructionStringEnd, &pLineNext);

		// Read the instruction name into pCommand.
		memset(pCommand, 0, sizeof(pCommand));

		for(nCommandCount = 0; (nCommandCount < kMaxCommandCount) && EA::StdC::SplitTokenSeparated(pLine, pLineEnd - pLine, (char8_t)' ', &pCommand[nCommandCount][0], kMaxCommandSize, &pLine); ++nCommandCount)
		{
		...
		if(nCommandCount)
		{
			if(EA::StdC::Stricmp(pCommand[0], "SetExpansion") == 0)
			{
				if(nCommandCount < 3)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEISetExpansion;
				pInstructionList[instructionCount++].mU32 = EA::StdC::AtoU32(pCommand[1]);
				pInstructionList[instructionCount++].mU32 = EA::StdC::AtoU32(pCommand[2]);
			}
			else if(EA::StdC::Stricmp(pCommand[0], "SetGlyphMinAlpha") == 0)
			{
				if(nCommandCount < 2)
					nErrorLine = nCurrentLine;
			...
			else if(EA::StdC::Stricmp(pCommand[0], "SetGlyphColor") == 0)
			{
				if(nCommandCount < 2)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEISetGlyphColor;
				pInstructionList[instructionCount++].mU32 = EA::StdC::StrtoU32(pCommand[1], NULL, 16);
			}
			else if(EA::StdC::Stricmp(pCommand[0], "SetGlyphOffset") == 0)
			{
				if(nCommandCount < 3)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEISetGlyphOffset;
				pInstructionList[instructionCount++].mI32 = EA::StdC::AtoI32(pCommand[1]);
				pInstructionList[instructionCount++].mI32 = EA::StdC::AtoI32(pCommand[2]);
			}
			else if(EA::StdC::Stricmp(pCommand[0], "SetGlyphHSpace") == 0)
			{
				if(nCommandCount < 3)
					nErrorLine = nCurrentLine;
			...
			else if(EA::StdC::Stricmp(pCommand[0], "SetGlyphVSpace") == 0)
			{
				if(nCommandCount < 3)
					nErrorLine = nCurrentLine;
			...
			else if(EA::StdC::Stricmp(pCommand[0], "SetGlyphSmooth") == 0)
			{
				if(nCommandCount < 2)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32  = kEISetGlyphSmooth;
				pInstructionList[instructionCount++].mBool = EA::StdC::AtoU32(pCommand[1]) ? 1 : 0;
			}
			else if(EA::StdC::Stricmp(pCommand[0], "SetGlyphBrush") == 0)
			{
				if(nCommandCount < 5)
					nErrorLine = nCurrentLine;
				...
				pInstructionList[instructionCount++].mU32 = EA::StdC::AtoU32(pCommand[1]);
				pInstructionList[instructionCount++].mU32 = EA::StdC::AtoU32(pCommand[2]);
				pInstructionList[instructionCount++].mF32 = (float)EA::StdC::Atof(pCommand[3]);
				pInstructionList[instructionCount++].mF32 = (float)EA::StdC::Atof(pCommand[4]);
			}
			else if(EA::StdC::Stricmp(pCommand[0], "SetTargetImage") == 0)
			{
				if(nCommandCount < 2)
					nErrorLine = nCurrentLine;
			...
			else if(EA::StdC::Stricmp(pCommand[0], "SetTargetMask") == 0)
			{
				if(nCommandCount < 2)
					nErrorLine = nCurrentLine;
			...
			else if(EA::StdC::Stricmp(pCommand[0], "DrawGlyphSmearOutline") == 0)
			{
				if(nCommandCount < 1)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEIDrawGlyphSmearOutline;
			}
			else if(EA::StdC::Stricmp(pCommand[0], "DrawGlyphBrushOutline") == 0)
			{
				if(nCommandCount < 1)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEIDrawGlyphBrushOutline;
			}
			else if(EA::StdC::Stricmp(pCommand[0], "DrawGlyph") == 0)
			{
				if(nCommandCount < 1)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEIDrawGlyph;
			}
			else if(EA::StdC::Stricmp(pCommand[0], "Clear") == 0)
			{
				if(nCommandCount < 1)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEIClear;
			}
			else if(EA::StdC::Stricmp(pCommand[0], "ApplyCurve") == 0)
			{
				if(nCommandCount < 3)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEIApplyCurve;
				pInstructionList[instructionCount++].mU32 = EA::StdC::AtoU32(pCommand[1]);
				pInstructionList[instructionCount++].mF32 = (float)EA::StdC::Atof(pCommand[2]);
			}
			else if(EA::StdC::Stricmp(pCommand[0], "ApplyBoxBlur") == 0)
			{
				if(nCommandCount < 4)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEIApplyBoxBlur;
				pInstructionList[instructionCount++].mF32 = (float)EA::StdC::Atof(pCommand[1]);
				pInstructionList[instructionCount++].mF32 = (float)EA::StdC::Atof(pCommand[2]);
				pInstructionList[instructionCount++].mU32 = EA::StdC::AtoU32(pCommand[3]);
			}
			else if(EA::StdC::Stricmp(pCommand[0], "Merge") == 0)
			{
				if(nCommandCount < 1)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEIMerge;
			}
			else if(EA::StdC::Stricmp(pCommand[0], "CallUser") == 0)
			{
				if(nCommandCount < 2)
					nErrorLine = nCurrentLine;
				pInstructionList[instructionCount++].mU32 = kEICallUser;
				pInstructionList[instructionCount++].mU32 = EA::StdC::StrtoU32(pCommand[1], NULL, 0);
			}
			else
			{
				if(EA::StdC::Strstr(pCommand[0], "//") != pCommand[0]) // If the line doesn't begin with a C++-like comment...
					nErrorLine = nCurrentLine;
	...
	mGlyphMetrics()
{
	...
	mEffectsBitmaps.mUserCount.Increment();
}


EffectsState::~EffectsState()
{
	if(mEffectsBitmaps.mUserCount.Decrement() == 0) // If we are the last user...
	{
...
bool EffectsProcessor::SetCurrentGlyphState(GlyphState glyphState)
{
	...
	if(glyphStateCurrent != glyphState)
	{
		if(glyphState != kGlyphNotDrawn)
		{
			...
			// We don't currently have support for SubPixelAA with effects.
			...
			if(nFTError == 0)
			{
				const FT_Render_Mode renderMode = ((glyphState == kGlyphDrawnMono) ? FT_RENDER_MODE_MONO : FT_RENDER_MODE_NORMAL); // FT_RENDER_MODE_NORMAL, FT_RENDER_MODE_LIGHT, FT_RENDER_MODE_MONO, FT_RENDER_MODE_LCD
...
void EffectsProcessor::AdjustFontMetrics(FontMetrics& fontMetrics)
{
	...
	void EffectsProcessor::DebugPrint()
	{
		...
		EA::StdC::Sprintf(filePath, "C:\\temp\\glyph\\SrcImage_%u.ppm", mEffectsState.mpCurrentInstruction - mEffectsState.mInstructionList);
		mEffectsState.mEffectsBitmaps.mFloatImage.WritePPMFile(filePath, 0x00);

		EA::StdC::Sprintf(filePath, "C:\\temp\\glyph\\DestImage_%u.ppm", mEffectsState.mpCurrentInstruction - mEffectsState.mInstructionList);
