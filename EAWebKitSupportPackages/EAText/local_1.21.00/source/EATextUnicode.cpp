...
// GetCharName
//
EATEXT_API uint32_t GetCharName(Char c, Char* pName, uint32_t nameCapacity, const UnicodeOptions* pUnicodeOptions)
{
	#if EA_WINAPI_FAMILY_PARTITION(EA_WINAPI_PARTITION_DESKTOP)
	{
		...
		typedef DWORD(__stdcall *GetUNameFunc)(uint16_t nUnicodeChar, WCHAR* pName);
		...
		if(pGetUName && (nameCapacity >= 256))
		{
			EA_ASSERT(!pUnicodeOptions || !pUnicodeOptions->mbNoNullTerminate); EA_UNUSED(pUnicodeOptions); // We don't have a means to use pUnicodeOptions without doing a double copy.

			#if (EATEXT_CHAR_TYPE_SIZE > 2)
				if(c <= UINT16_MAX)
				{
					WCHAR buffer[256]; buffer[0] = 0;
					pGetUName((uint16_t)c, buffer);
					return EA::StdC::Strlcpy(pName, buffer, nameCapacity);
				}
				// Else fall through
			#else
				return (uint32_t)pGetUName(c, reinterpret_cast<WCHAR*>(pName));
			#endif
		}
	}
	#else
		// One option is to let the user register a file path to a unicode names text file,
		// such as the one referred to above. It remains to be decided what the best way 
		// of dealing with that data is. Should we cache results? Convert the file to a hash_map?
		// Current proposed solution: Store all names <= the highest requested value in a hash map,
		// and save the file position of the next line so we can load more if needed as we go.
		// Also, supply a UnicodeNames.txt file with the EAText package as a data file.
	#endif
	...
	EA_COMPILETIME_ASSERT((EATEXT_CHAR_TYPE_SIZE == 2) || (EATEXT_CHAR_TYPE_SIZE == 4));
	const char* kFormat = (EATEXT_CHAR_TYPE_SIZE == 2) ? "0x%04x" : "0x%08x";
	uint32_t    requiredStrlen = (2 + (EATEXT_CHAR_TYPE_SIZE * 2)); // 6 or 10, for 0xhhhh or 0xhhhhhhhh
	...
	EA::StdC::Sprintf(buffer, kFormat, (unsigned)c);

	for(i = 0, j = 0; i < requiredStrlen; i++) // 0xhhhh or 0xhhhhhhhh
	{
		if((j + nullTermination) < nameCapacity)
			pName[j++] = (uint8_t)buffer[i];
	}

	if(nullTermination && (j < nameCapacity))
		pName[j] = 0;
...
EATEXT_API BidiClass GetBidiClass(Char c)
{
	...
	// Specifies that the following text is to be explicitly left to right or right to left.
	if(c == kCharLRM)
		return kBidiClassL;
	else if(c == kCharRLM)
		return kBidiClassR;
...
EATEXT_API Block GetCharBlock(Char c)
{
	...
	while(nLower <= nUpper)
	{
		...
		if(c < gBlockTableEntryArray[nMid].mnBegin)
			nUpper = nMid - 1;
		else if(c >= (Char)(gBlockTableEntryArray[nMid].mnBegin + gBlockTableEntryArray[nMid].mnCount))
			nLower = nMid + 1;
		else
...
EATEXT_API int GetDirection(const Char* pText, uint32_t nTextLength)
{
	...
	EA_DISABLE_VC_WARNING(4062) // enumerate 'kBidiClassON' in switch of enum 'EA::Text::BidiClass' is not handled
	...
	EA_RESTORE_VC_WARNING(); // for warning 4062 above
	...
	if((c >= 0x0300) && (c < 0x3100))
	{
		...
		while(nLower <= nUpper)
		{
			...
			if(c < gCombiningClassEntryArray[nMid].mnBegin)
				nUpper = nMid - 1;
			else if(c >= (Char)(gCombiningClassEntryArray[nMid].mnBegin + gCombiningClassEntryArray[nMid].mnCount))
				nLower = nMid + 1;
			else
...
// See http://icu-project.org/docs/papers/optimized_unicode_composition_and_decomposition.html
//
EATEXT_API uint32_t ASCIICompatibleEncoding(const Char* pTextInput, uint32_t nTextLength, 
