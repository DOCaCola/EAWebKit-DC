...
#if EACALLSTACK_LLVM_DEMANGLE_AVAILABLE
...
#if (defined(__GNUC__) || defined(__clang__)) && !defined(__EDG_VERSION__)
...
class __typeinfo_class
	: public __node
{
	static const ptrdiff_t n = sizeof("typeinfo for ") - 1;
public:
	__typeinfo_class(__node* type)
	{
		__right_ = type;
	}
...
class __static_cast_class
	: public __node
{
public:

	__static_cast_class(__node* op1, __node* op2)
	{
		__left_ = op1;
		__right_ = op2;
	}
...
class __wchar_t_type // Changed from __wchar_t to __wchar_t_type by Paul Pedriana, as __wchar_t collides with some compilers' own __wchar_t.
	: public __node
{
...
__demangle_tree::__parse_builtin_type(const char* first, const char* last)
{
	if (first != last)
	{
		...
		case 'v':
			...
			break;
		case 'w':
			if (__make<__wchar_t_type>()) // Used to be __wchar_t
				++first;
			break;
		case 'b':
			if (__make<__bool_type>())
				++first;
			break;
		case 'c':
			if (__make<__char>())
		...
		if (t != last)
		{
			...
			if (t1 != t)
			{
				...
				if (t1 != t && t1 != last && *t1 == 'E')
				{
					...
					if (static_cast<__void*>(__root_->__left_) != NULL)
...
{
	const char* t = first;
	for (; t != last && isxdigit((unsigned char)*t); ++t)
	{
		if (t == first)
			n = 0;
		...
		case 'f':
			{
				...
				if (t != first+2 && t != last && *t == 'E')
				{
					...
					fi.i = static_cast<unsigned>(j);
					if (__make<__float_literal>(fi.f))
						first = t+1;
				}
			}
			break;
		case 'd':
			{
				if (last - (first+2) <= 16)
					return first;
				DoubleUint64 du = { 0.0 };
				const char* t = __parse_hex_number(first+2, first+18, du.j);
				if (t != first+2 && t != last && *t == 'E')
				{
					if (__make<__double_literal>(du.d))
						first = t+1;
				}
			}
			break;
		...
		default:
			{
				...
				if (t != first+1 && t != last)
				{
					...
					{
						const char* n = t;
						for (; n != last && isdigit((unsigned char)*n); ++n)
							;
						if (n != t && n != last && *n == 'E')
						{
...
__demangle_tree::__parse_discriminator(const char* first, const char* last)
{
	...
	if (first != last)
	{
		if (*first == '_')
		{
			...
			if (t1 != last)
			{
				if (isdigit((unsigned char)*t1))
					first = t1+1;
				else if (*t1 == '_')
				{
					for (++t1; t1 != last && isdigit((unsigned char)*t1); ++t1)
						;
					if (t1 != last && *t1 == '_')
						first = t1 + 1;
				}
		...
		else if (isdigit((unsigned char)*first))
		{
			const char* t1 = first+1;
			for (; t1 != last && isdigit((unsigned char)*t1); ++t1)
				;
			first = t1;
		}
	}
...
__demangle_tree::__parse_static_cast_expr(const char* first, const char* last)
{
	if (last - first >= 3 && first[0] == 's' && first[1] == 'c')
	{
		...
		if (t != first+2)
		{
			...
			if (t1 != t)
			{
				if (__make<__static_cast_class>(type, __root_))
					first = t1;
			}
		}
	}
...
__demangle_tree::__parse_array_type(const char* first, const char* last)
{
	if (first != last && *first == 'A' && first+1 != last)
	{
		...
		else if ('1' <= first[1] && first[1] <= '9')
		{
			...
			for (; t != last && isdigit((unsigned char)*t); ++t)
...
__demangle_tree::__parse_template_param(const char* first, const char* last)
{
	if (last - first >= 2)
	{
		if (*first == 'T')
		{
			...
			else if (isdigit((unsigned char)first[1]))
			{
				const char* t = first+1;
				size_t sub = static_cast<size_t>(*t - '0');
				for (++t; t != last && isdigit((unsigned char)*t); ++t)
				{
					sub *= 10;
...
__demangle_tree::__parse_vector_type(const char* first, const char* last)
{
	if (last - first > 3 && first[0] == 'D' && first[1] == 'v')
	{
		if ('1' <= first[2] && first[2] <= '9')
		{
			...
			while (*t != '_')
			{
				if (!isdigit((unsigned char)*t) || ++t == last)
					return first;
			}
...
__demangle_tree::__parse_number(const char* first, const char* last)
{
	if (first != last)
	{
		...
		if (t != last)
		{
			...
			{
				first = t+1;
				while (first != last && isdigit((unsigned char)*first))
					++first;
			}
		}
	}
	...
	if (last - first > 2)
	{
		...
		case 'T':
			...
			case 'I':
				// TI <type>    # typeinfo structure
				t = __parse_type(first+2, last);
				if (t != first+2 && __make<__typeinfo_class>(__root_))
					first = t;
				break;
			case 'S':
				// TS <type>    # typeinfo name (null-terminated byte string)
	...
	if (last - first >= 2)
	{
		if (*first == 'S')
		{
			...
			case '_':
				...
				break;
			default:
				if (isdigit((unsigned char)first[1]) || isupper((unsigned char)first[1]))
				{
					size_t sub = 0;
					const char* t = first+1;
					if (isdigit((unsigned char)*t))
					...
					for (++t; t != last && (isdigit((unsigned char)*t) || isupper((unsigned char)*t)); ++t)
					{
						sub *= 36;
						if (isdigit((unsigned char)*t))
							sub += static_cast<size_t>(*t - '0');
	...
	if (first != last)
	{
		...
		if (t == t0)
		{
			...
			if (t == t0)
			{
				...
				{
					if (t0[0] == 'S' && (t0[1] == '_'   ||
											isdigit((unsigned char)t0[1]) ||
											isupper((unsigned char)t0[1]) ||
											t0[1] == 'a'   ||
											t0[1] == 'b'))
					{
...
printf("\n");
	...
	{
		char* newBuf = static_cast<char*>(realloc(buf, static_cast<size_t>(s)));
		if (newBuf == NULL)
		{
			free(buf);
			buf = NULL;
		...
		// we need to assign to buf since it is used later in this function
		buf = newBuf;
		if (n)
...
char*
llvm_cxa_demangle(const char* mangled_name, char* buf, size_t* n, int* status)
{
	...
	const size_t bsExtra = bs + 16;

	char  tree_buf[bsExtra];
	char* tree_buf_aligned = (char*)((intptr_t)(tree_buf + 15) & ~(intptr_t)15);

	buf = __libcxxabi::__demangle(__libcxxabi::__demangle(mangled_name, tree_buf_aligned, bs), buf, n, status);
