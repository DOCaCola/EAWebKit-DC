...
union FloatUint32Adapter
{
	float    f32;
	uint32_t u32;
};

union DoubleUint64Adapter
{
	double   d64;
	uint64_t u64;
};
...
{
	FloatUint32Adapter fu;

	if(ReadUint32(pIS, fu.u32, endianSource))
	{
		value = fu.f32;
		return true;
	}
...
EAIO_API bool EA::IO::ReadFloat(IStream* pIS, float* valueArray, size_type count, Endian endianSource)
{
	return ReadUint32(pIS, reinterpret_cast<uint32_t*>(reinterpret_cast<uintptr_t>(valueArray)), count, endianSource);
}


EAIO_API bool EA::IO::ReadDouble(IStream* pIS, double& value, Endian endianSource)
{
	DoubleUint64Adapter fu;

	if(ReadUint64(pIS, fu.u64, endianSource))
	{
		value = fu.d64;
		return true;
	}
...
EAIO_API bool EA::IO::ReadDouble(IStream* pIS, double* valueArray, size_type count, Endian endianSource)
{
	return ReadUint64(pIS, reinterpret_cast<uint64_t*>(reinterpret_cast<uintptr_t>(valueArray)), count, endianSource);
}
...
EAIO_API EA::IO::size_type EA::IO::ReadString(IStream* pIS, char* pBuffer, size_type nMaxCount, EA::IO::Endian endianSource)
{
...
EAIO_API EA::IO::size_type EA::IO::ReadString(IStream* pIS, char32_t* pBuffer, size_type nMaxCount, EA::IO::Endian endianSource)
{
	const off_type nInitialPosition(pIS->GetPosition());
	...
	while(pBuffer && (nCount < nReadLength)) 
	{
		if(!EA::IO::ReadUint32(pIS, (uint32_t&)cCurrent, endianSource))
			break;
	...
	// due to the nMaxCount limit, but we still want to advance the stream's
	// position to the end of the string.
	pIS->SetPosition(nInitialPosition + (off_type)sizeof(uint32_t) + (off_type)(nLength * sizeof(char32_t)));
...
EAIO_API EA::IO::size_type EA::IO::ReadLine(IStream* pIS, char* pLine, size_type nMaxCount, Endian /*unused*/)
{
	char   cCurrent;
...
EAIO_API EA::IO::size_type EA::IO::ReadLine(IStream* pIS, char32_t* pLine, size_type nMaxCount, Endian endianSource)
{
	char32_t  cCurrent;
	size_type nCount(0); // Number of chars in the line, not including the line end characters(s).
	size_type nRead(0);  // Number of chars successfully read from stream. Will be >= nCount (due to presence of newlines).
	...
	for(;;)
	{
		...
		if(nResult == sizeof(cCurrent))
		{
			...
			else
			{
				...
				{
					if(endianSource != kEndianLocal)
						cCurrent = SwizzleUint32(cCurrent);
...
EAIO_API bool EA::IO::WriteString(IStream* pOS, const char* pBuffer, size_t nCount, Endian endianSource)
{
	bool bResult(true);
	...
	if(bResult && nCount)
		bResult = pOS->Write(pBuffer, nCount * sizeof(char));
...
EAIO_API bool EA::IO::WriteString(IStream* pOS, const char32_t* pBuffer, size_t nCount, Endian endianSource)
{
	bool bResult(true);
	...
	if(nCount == kLengthNull)
	{
		...
		// For maximal portability and modularity, we invent our own strlen function here.
		const char32_t* pCurrent = pBuffer;
		while(*pCurrent++)
			++nCount;
	}
	...
	if(bResult && nCount)
		bResult = WriteUint32(pOS, (const uint32_t*)pBuffer, nCount, endianSource);
...
EAIO_API bool EA::IO::WriteLine(IStream* pOS, const char* pLine, size_type nCount, LineEnd lineEndToUse)
{
	bool bResult(true);
	...
	if(nCount)
		bResult = pOS->Write(pLine, nCount * sizeof(char));
...
EAIO_API bool EA::IO::WriteLine(IStream* pOS, const char32_t* pLine, size_type nCount, LineEnd lineEndToUse, Endian endianDestination)
{
	bool bResult(true);
	...
	if(nCount == kLengthNull)
	{
		...
		// For maximial portability and modularity, we invent our own strlen function here for char32_t.
		const char32_t* pCurrent = pLine;
		while(*pCurrent++)
			++nCount;
	}

	if(nCount)
		bResult = WriteUint32(pOS, (const uint32_t*)pLine, nCount, endianDestination);
	...
	if(bResult)
	{
		...
		if(lineEndToUse == kLineEndWindows)
		{
			const uint32_t terminator[2]= { '\r', '\n' };
			bResult = WriteUint32(pOS, terminator, 2, endianDestination);

		}
		else if(lineEndToUse == kLineEndNewline)
		{
			const uint32_t terminator[1]= { '\n' };
			bResult = WriteUint32(pOS, terminator, endianDestination);
		}
	}
...
EAIO_API void EA::IO::EAStreamAdapterAssertionFailure()
{
	EA_FAIL_MSG("EAIO EAStreamAdapter assertion failure!");
}
