...
namespace EA
{
	namespace IO
	{
		...
		///    void MyCallback(FileChangeNotification* pFCN, const wchar_t* pDirectoryPath,
		...
		typedef void (*FileChangeCallback)(FileChangeNotification* pFileChangeNotification, /// The class object which generated this callback.
											const wchar_t* pDirectoryPath,                  /// The directory in which the change took place, which will the registered directory or a subdirectory of it.
											const wchar_t* pFileName,                       /// The file name (not path) of the file that changed. Will be NULL if the change was a directory change. Can be NULL if the file which changed could not be determined.
		...
		///         void Init() {
		///             mFCN.Init();
		///             mFCN.SetWatchedDirectory(EA_CHAR16("/app/data/")));
		...
		public:
			...
			///
			const wchar_t* GetWatchedDirectory() const;
			...
			///
			bool SetWatchedDirectory(const wchar_t* pDirectory);
			bool SetWatchedDirectory(const char* pDirectory);
			...
			/// the specification of and manipulation of paths.
			///
			void SendNotification(const wchar_t* pDirectory, const wchar_t* pFile, int nChangeTypeFlags);
		...
		protected:
			// Member functions and data
				...
				void ThreadFunctionFFCN(uint32_t dwNotifyFilter, const wchar_t* pWatchedDirectory);  ///< Version of thread that uses Win32's FindFirstChangeNotification().
			...
			//     https://developer.apple.com/library/mac/#documentation/Darwin/Conceptual/FSEvents_ProgGuide/UsingtheFSEventsFramework/UsingtheFSEventsFramework.html
				...
				typedef eastl::fixed_string<wchar_t, 32, true, Allocator::EAIOEASTLCoreAllocator> EntryNameString; // Most directory name entries fit within this many characters.
				...
				void     BuildEntry(FSEntry* pEntry, const wchar_t* pName, FSEntry* pEntryParent, bool bDirectory);
				void     GetEntryPath(const FSEntry* pEntry, wchar_t* pPath, uint32_t nMaxPermittedLength = kMaxPathLength);
			...
			wchar_t                     mDirectoryPath[kMaxDirectoryLength];
		...
		#if FCN_POLL_ENABLED && EAIO_THREAD_SAFETY_ENABLED
			#if defined(_MSC_VER)
				#pragma warning(pop)
			#endif
		#endif
