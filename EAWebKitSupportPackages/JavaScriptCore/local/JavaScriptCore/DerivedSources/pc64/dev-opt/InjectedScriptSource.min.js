...
{return InjectedScript.primitiveTypes[typeof object]&&!this._isHTMLAllCollection(object);},wrapObject:function(object,groupName,canAccessInspectedGlobalObject,generatePreview)
...
return result;},_getProperties:function(objectId,collectionMode,generatePreview)
...
return false;var descriptors=this._propertyDescriptors(object,collectionMode);for(var i=0;i<descriptors.length;++i){var descriptor=descriptors[i];if("get"in descriptor)
...
{var collectionMode=ownProperties?InjectedScript.CollectionMode.OwnProperties:InjectedScript.CollectionMode.AllProperties;return this._getProperties(objectId,collectionMode,generatePreview);},getDisplayableProperties:function(objectId,generatePreview)
{var collectionMode=InjectedScript.CollectionMode.OwnProperties|InjectedScript.CollectionMode.NativeGetterProperties;return this._getProperties(objectId,collectionMode,generatePreview);},getInternalProperties:function(objectId,generatePreview)
{var parsedObjectId=this._parseObjectId(objectId);var object=this._objectForId(parsedObjectId);var objectGroupName=this._idToObjectGroupName[parsedObjectId.id];if(!this._isDefined(object))
return false;if(isSymbol(object))
...
return"Cannot resolve function by id.";var details=InjectedScriptHost.functionDetails(func);if(!details)
return"Cannot resolve function details.";if("rawScopes"in details){var objectGroupName=this._idToObjectGroupName[parsedFunctionId.id];var rawScopes=details.rawScopes;var scopes=[];delete details.rawScopes;for(var i=0;i<rawScopes.length;i++)
scopes.push(InjectedScript.CallFrameProxy._createScopeJson(rawScopes[i].type,rawScopes[i].object,objectGroupName));details.scopeChain=scopes;}
return details;},releaseObject:function(objectId)
...
{return this._evaluateAndWrap(InjectedScriptHost.evaluate,InjectedScriptHost,expression,objectGroup,false,injectCommandLineAPI,returnByValue,generatePreview,saveResult);},callFunctionOn:function(objectId,expression,args,returnByValue,generatePreview)
...
var parameters=[InjectedScriptHost.evaluate,expression];var expressionFunctionBody=""+"var global = Function('return this')() || (1, eval)('this');"+"var __originalEval = global.eval; global.eval = __eval;"+"try { return eval(__currentExpression); }"+"finally { global.eval = __originalEval; }";if(commandLineAPI){



var parameterNames=Object.getOwnPropertyNames(commandLineAPI);for(var i=0;i<parameterNames.length;++i)
parameters.push(commandLineAPI[parameterNames[i]]);var expressionFunctionString="(function(__eval, __currentExpression, "+parameterNames.join(", ")+") { "+expressionFunctionBody+" })";}else{
var expressionFunctionString="(function(__eval, __currentExpression) { "+expressionFunctionBody+" })";}
var boundExpressionFunctionString="(function(__function, __thisObject) { return function() { return __function.apply(__thisObject, arguments) }; })("+expressionFunctionString+", this)";var expressionFunction=evalFunction.call(object,boundExpressionFunctionString);var result=expressionFunction.apply(null,parameters);if(saveResult)
this._saveResult(result);return result;}

try{if(commandLineAPI){if(inspectedGlobalObject.console)
inspectedGlobalObject.console.__commandLineAPI=commandLineAPI;else
inspectedGlobalObject.__commandLineAPI=commandLineAPI;expression="with ((this && (this.console ? this.console.__commandLineAPI : this.__commandLineAPI)) || {}) { "+expression+"\n}";}
var result=evalFunction.call(inspectedGlobalObject,expression);if(saveResult)
this._saveResult(result);return result;}finally{if(commandLineAPI){if(inspectedGlobalObject.console)
delete inspectedGlobalObject.console.__commandLineAPI;else
delete inspectedGlobalObject.__commandLineAPI;}}},wrapCallFrames:function(callFrame)
{if(!callFrame)
...
return"Could not find call frame with given id";return this._evaluateAndWrap(callFrame.evaluate,callFrame,expression,objectGroup,true,injectCommandLineAPI,returnByValue,generatePreview,saveResult);},_callFrameForId:function(topCallFrame,callFrameId)
...
return descriptors;},_propertyDescriptors:function(object,collectionMode)
{var descriptors=[];var nameProcessed=new Set;function createFakeValueDescriptor(name,symbol,descriptor,isOwnProperty,possibleNativeBindingGetter)
...
descriptor.symbol=symbol;return descriptor;}catch(e){var errorDescriptor={name,value:e,wasThrown:true};if(isOwnProperty)
...
if(possibleNativeBindingGetter){descriptors.push(descriptor);return;}}}
...
var fakeDescriptor=createFakeValueDescriptor(name,symbol,descriptor,isOwnProperty,true);processDescriptor(fakeDescriptor,isOwnProperty,true);continue;}
...
{var array=new Array(length);for(var i=0;i<length;++i){if(i in o)
array.push(""+i);}
return array;}

var isArrayTypeWithLargeLength=false;try{isArrayTypeWithLargeLength=injectedScript._subtype(object)==="array"&&isFinite(object.length)&&object.length>100;}catch(e){}
for(var o=object;this._isDefined(o);o=o.__proto__){var isOwnProperty=o===object;if(isArrayTypeWithLargeLength&&isOwnProperty)
processProperties(o,arrayIndexPropertyNames(o,100),isOwnProperty);else{processProperties(o,Object.getOwnPropertyNames(o),isOwnProperty);if(Object.getOwnPropertySymbols)
...
return null;},_nodeDescription:function(node)
{var isXMLDocument=node.ownerDocument&&!!node.ownerDocument.xmlVersion;var description=isXMLDocument?node.nodeName:node.nodeName.toLowerCase();switch(node.nodeType){case 1: if(node.id)
description+="#"+node.id;if(node.hasAttribute("class")){description+="."+node.getAttribute("class").trim().replace(/\s+/g,".");}
return description;default:return description;}},_classPreview:function(classConstructorValue)
...
return toString(obj);if(subtype==="node")
return this._nodeDescription(obj);var className=InjectedScriptHost.internalConstructorName(obj);if(subtype==="array")
return className;if(typeof obj==="function")
return toString(obj);if(className==="Object"){var constructorName=obj.constructor&&obj.constructor.name;if(constructorName)
...
InjectedScript.RemoteObject.prototype={_initialPreview:function()
...
return preview;},_createObjectPreviewForValue:function(value,generatePreview)
{var remoteObject=new InjectedScript.RemoteObject(value,undefined,false,generatePreview,undefined);if(remoteObject.objectId)
injectedScript.releaseObject(remoteObject.objectId);if(remoteObject.classPrototype&&remoteObject.classPrototype.objectId)
injectedScript.releaseObject(remoteObject.classPrototype.objectId);return remoteObject.preview||remoteObject._emptyPreview();},_generatePreview:function(object,firstLevelKeys,secondLevelKeys)
{var preview=this._initialPreview();if(this.type!=="object")
return;var isTableRowsRequest=secondLevelKeys===null||secondLevelKeys;var firstLevelKeysCount=firstLevelKeys?firstLevelKeys.length:0;var propertiesThreshold={properties:isTableRowsRequest?1000:Math.max(5,firstLevelKeysCount),indexes:isTableRowsRequest?1000:Math.max(10,firstLevelKeysCount)};try{if(this.subtype==="map"||this.subtype==="set"||this.subtype==="weakmap"||this.subtype==="weakset"||this.subtype==="iterator")
...
return preview;}
if(preview.entries)
return preview;var descriptors=injectedScript._propertyDescriptors(object,InjectedScript.CollectionMode.AllProperties);this._appendPropertyPreviews(object,preview,descriptors,false,propertiesThreshold,firstLevelKeys,secondLevelKeys);if(propertiesThreshold.indexes<0||propertiesThreshold.properties<0)
...
{for(var descriptor of descriptors){if(propertiesThreshold.indexes<0||propertiesThreshold.properties<0)
break;if(descriptor.wasThrown){preview.lossless=false;continue;}
var name=descriptor.name;if(name==="__proto__")
continue;if(this.subtype==="array"&&!isUInt32(name))
continue;if(!descriptor.enumerable&&!descriptor.isOwn&&this.subtype!=="array")
...
property.subtype=subtype;if((secondLevelKeys===null||secondLevelKeys)||this._isPreviewableObject(value,object)){var subPreview=this._createObjectPreviewForValue(value,value!==object);property.valuePreview=subPreview;if(!subPreview.lossless)
...
preview.entries=entries.map(function(entry){entry.value=this._createObjectPreviewForValue(entry.value,entry.value!==object);updateMainPreview(entry.value);if("key"in entry){entry.key=this._createObjectPreviewForValue(entry.key,entry.key!==object);updateMainPreview(entry.key);}
return entry;},this);},_isPreviewableObject:function(value,object)
{return this._isPreviewableObjectInternal(value,new Set([object]),1);},_isPreviewableObjectInternal:function(object,knownObjects,depth)
{if(depth>3)
...
return false;var ownPropertyNames=Object.getOwnPropertyNames(object);if(ownPropertyNames.length>3)
return false;for(var propertyName of ownPropertyNames){if(!this._isPreviewableObjectInternal(object[propertyName],knownObjects,depth))
return false;}
...
{var scopeChain=callFrame.scopeChain;var scopeChainProxy=[];for(var i=0;i<scopeChain.length;i++)
scopeChainProxy[i]=InjectedScript.CallFrameProxy._createScopeJson(callFrame.scopeType(i),scopeChain[i],"backtrace");return scopeChainProxy;}}
InjectedScript.CallFrameProxy._scopeTypeNames={0:"global", 1:"with", 2:"closure", 3:"catch", 4:"functionName", 5:"globalLexicalEnvironment", 6:"nestedLexical",}
InjectedScript.CallFrameProxy._createScopeJson=function(scopeTypeCode,scopeObject,groupId)
{return{object:injectedScript._wrapObject(scopeObject,groupId),type:InjectedScript.CallFrameProxy._scopeTypeNames[scopeTypeCode]};}
function slice(array,index)
{var result=[];for(var i=index||0;i<array.length;++i)
result.push(array[i]);return result;}
function bind(func,thisObject,var_args)
{var args=slice(arguments,2);return function(var_args){return func.apply(thisObject,args.concat(slice(arguments)));}}
function BasicCommandLineAPI()
{this.$_=injectedScript._lastResult;this.$exception=injectedScript._exceptionValue; for(var i=1;i<=injectedScript._savedResults.length;++i){var member="$"+i;if(member in inspectedGlobalObject)
