...
extern SceKernelCpumask GetSceKernelAllCpuMask();
...
namespace
{
	...
	void ConvertToNativePriority(int eathreadPriority, sched_param& param, int& policy)
	{
		...
		// Sony pthread_disableds uses a reversed interpretation of sched_get_priority_min and sched_get_priority_max.
...
// This function is not currently used if the thread name can be set from any other thread
	...
	static void SetPlatformThreadAffinity(EAThreadDynamicData* pTDD)
	{
		...
		{
			if(pTDD->mStartupProcessor == EA::Thread::kProcessorAny)
			{
				if(pTDD->mnThreadAffinityMask == EA::Thread::kThreadAffinityMaskAny)
					scePthreadSetaffinity(pTDD->mSysThreadId, GetSceKernelAllCpuMask());
				else
					scePthreadSetaffinity(pTDD->mSysThreadId, pTDD->mnThreadAffinityMask & GetSceKernelAllCpuMask());
			}
			else if(pTDD->mStartupProcessor != EA::Thread::kProcessorDefault)
			{
				SceKernelCpumask mask = (1 << pTDD->mStartupProcessor) & GetSceKernelAllCpuMask();
				auto nResult = scePthreadSetaffinity(pTDD->mSysThreadId, mask);
	...
	if(pData)
	{
		...
		if(pData->mStartupProcessor == EA::Thread::kProcessorAny)
		{
			if(pData->mnThreadAffinityMask == kThreadAffinityMaskAny)
				scePthreadAttrSetaffinity(&creationAttribs, GetSceKernelAllCpuMask());
			else
				scePthreadAttrSetaffinity(&creationAttribs, pData->mnThreadAffinityMask & GetSceKernelAllCpuMask());
		}
		else if(pData->mStartupProcessor != kProcessorDefault)
		{
			SceKernelCpumask mask = (1 << pData->mStartupProcessor) & GetSceKernelAllCpuMask();
