...
int EA::Thread::GetThreadPriority()
{
	...
	if(result == SCE_OK)
	{
		// Sony pthreads uses a reversed interpretation of sched_get_priority_min and sched_get_priority_max.
...
SceKernelCpumask GetSceKernelAllCpuMask()
{
	...
	#else
		return SCE_KERNEL_CPUMASK_13CPU;
	#endif
}
...
EATHREADLIB_API void EA::Thread::SetThreadAffinityMask(const EA::Thread::ThreadId& id, ThreadAffinityMask nAffinityMask)
{
	...
	const auto result = scePthreadSetaffinity(GetSysThreadId(id), static_cast<SceKernelCpumask>(nAffinityMask));
	EAT_ASSERT_FORMATTED(result == SCE_OK, "scePthreadSetaffinity returned an error (0x%08x)", result);
	EA_UNUSED(result);
#endif
}
...
#else
	return 16 - 3;  // 3 used by Sony, 13 for us.
#endif
}
