...
bool JSSQLTransactionErrorCallback::handleEvent(SQLError* error)
{
    if (!canInvokeCallback())
        return true;

    Ref<JSSQLTransactionErrorCallback> protectedThis(*this);

    JSLockHolder lock(m_data->globalObject()->vm());

    ExecState* state = m_data->globalObject()->globalExec();
    MarkedArgumentBuffer args;
    args.append(toJS<IDLInterface<SQLError>>(*state, *m_data->globalObject(), error));
...
JSC::JSValue toJS(SQLTransactionErrorCallback& impl)
{
    if (!static_cast<JSSQLTransactionErrorCallback&>(impl).callbackData())
        return jsNull();
