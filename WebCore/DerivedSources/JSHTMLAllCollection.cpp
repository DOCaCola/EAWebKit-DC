...
static const HashTable JSHTMLAllCollectionTable = { 2, 3, true, JSHTMLAllCollectionTableValues, JSHTMLAllCollectionTableIndex };
...
template<> const ClassInfo JSHTMLAllCollectionConstructor::s_info = { "HTMLAllCollectionConstructor", &Base::s_info, 0, CREATE_METHOD_TABLE(JSHTMLAllCollectionConstructor) };
...
JSHTMLAllCollection::JSHTMLAllCollection(Structure* structure, JSDOMGlobalObject& globalObject, Ref<HTMLAllCollection>&& impl)
    : JSDOMWrapper<HTMLAllCollection>(structure, globalObject, WTF::move(impl))
{
}
...
bool JSHTMLAllCollection::getOwnPropertySlot(JSObject* object, ExecState* state, PropertyName propertyName, PropertySlot& slot)
{
    ...
    if (getStaticValueSlot<JSHTMLAllCollection, Base>(state, JSHTMLAllCollectionTable, thisObject, propertyName, slot))
        return true;
...
EncodedJSValue jsHTMLAllCollectionLength(ExecState* state, JSObject* slotBase, EncodedJSValue thisValue, PropertyName)
{
    UNUSED_PARAM(state);
...
EncodedJSValue jsHTMLAllCollectionConstructor(ExecState* state, JSObject*, EncodedJSValue thisValue, PropertyName)
{
...
EncodedJSValue JSC_HOST_CALL jsHTMLAllCollectionPrototypeFunctionTags(ExecState* state)
{
    ...
    JSValue result = toJS(state, castedThis->globalObject(), WTF::getPtr(impl.tags(name)));
    return JSValue::encode(result);
}
...
JSC::JSValue toJSNewlyCreated(JSC::ExecState*, JSDOMGlobalObject* globalObject, HTMLAllCollection* impl)
{
    if (!impl)
        return jsNull();
