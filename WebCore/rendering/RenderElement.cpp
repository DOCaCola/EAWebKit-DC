...
void RenderElement::drawLineForBoxSide(GraphicsContext& graphicsContext, const FloatRect& rect, BoxSide side, Color color, EBorderStyle borderStyle, float adjacentWidth1, float adjacentWidth2, bool antialias) const
{
    ...
    case DASHED: {
		//+EAWebKitChange
		//06/17/2015 - Latest WebKit trunk has modified the code here which breaks our dotted/dashed borders, as it needs relevant changes in GraphicsContextCairo. The relevant WebKit changelist: https://bugs.webkit.org/show_bug.cgi?id=139872
		// Reverting this change, until GraphicsContextCairo is changed accordingly.
		{
			if (thickness > 0) {
